...
- 候选版本各项 `ratio` **不得超过与本机同比值画像的冻结 reference median × 该项容差**：读项 `status` / `diff` **130%**，写项 `restore` / `commit` **110%**。容差表里没有的测点判失败，不设默认值。
...
- `ubuntu-latest` 分到**没冻结过的 CPU 型号**同样是上一条：判失败，不降级。处理是重跑该 job 一次（托管 runner 多数时候落在已冻结的型号上）；同一新型号反复出现，就按下面的新画像冻结为它补一份。
...
- **已知带负载的基线复冻**是第三类合法理由，同样不属于「失败后重算基线」，条件是：
  1. 被替换的那份 reference 在冻结当时就有**可复验的负载证据**（如冻结记录里的 1 分钟负载、离群轮次），而不是「这次门禁红了」；
  2. 同样是 10 次独立运行、同一画像（`--regenerate` 覆盖已有的那份），理由里写明旧基线的负载证据与本次起跑时的负载；
  3. 所在提交在旧基线上**已经通过**相对门禁——复冻只校正基线里的噪声，不替一次失败的运行开脱。

**容差按读写分档**。读项 p95 只有两三毫秒，写项十几到几百毫秒，同一画像内的噪声差出一截。b1337e76 的 `bench-freeze` 四个槽位，每槽 10 次独立运行，各项 ratio 高出本槽 median 的最大幅度：

| 画像                                              | `status` | `diff` | `restore` | `commit` |
| ------------------------------------------------- | -------- | ------ | --------- | -------- |
| AMD EPYC 7763                                     | +19.3%   | +16.5% | +2.6%     | +4.6%    |
| Intel Xeon 6973P-C                                | +5.4%    | +19.7% | +7.4%     | +4.8%    |
| AMD EPYC 9V74（签入的槽位 4）                     | +9.9%    | +5.7%  | +7.6%     | +2.9%    |
| AMD EPYC 9V74 另一台主机（对槽位 4 的 reference） | +5.8%    | +25.1% | +7.7%     | +4.5%    |

读项按 110% 判，四组里有三组会在**没有任何代码变化**时报红（每组 2–4 轮）；CI run 36070569734 的实测对 EPYC 7763 reference 也是 `status` +14.7%、`diff` +11.8%。写项最大 +7.7%，110% 留得住。于是读项放到 130%，写项不动——统一放宽会让写项的真实回归漏过去，统一收紧则让读项的 PR 门禁看运气。
...
这道比对要成立，reference 自己必须记得它是在哪台机器上冻的——所以 `reference.runnerProfileHash` 是**必填**，而不是从当次运行的 `environment` 推断（那样永远相等，比对恒真）。`reference.regeneratedBecause` 只在重新冻结或新画像冻结时出现，记录 §3.1 允许的那三类理由（测点集合变化、新画像、已知带负载的基线复冻），使「为什么存在第二份基线」留在产物里而不是只留在提交记录里。
