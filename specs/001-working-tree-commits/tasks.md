...
- [ ] T130 全矩阵回归：`pnpm nx run-many -t test --projects=rxdb,rxdb-adapter-pglite,rxdb-adapter-wa-sqlite,rxdb-adapter-sqlite-wasm,rxdb-adapter-sqlite,rxdb-adapter-sqliteai,rxdb-adapter-electron` 确认 6 后端 × 2 套件双双全绿（SC-006）
  - **跑了，没绿，因此不打勾**（`--skip-nx-cache`，2m49s）。`rxdb:test` 与 12 个前置 build 全绿；6 个适配器**各红 1 条**，`rxdb-adapter-pglite:test-node` 也绿。合计 `1 failed | 5025 passed`：wa-sqlite 780/781、sqlite-wasm 754/755、sqlite 657/658、electron 974/975、sqliteai 665/666、pglite 1195/1196。
  - **6 条红是同一条断言**，逐字节相同：`commit.suite.ts §2.2 一次性启用迁移（US-305） > enable() 之后新建的分支自带 ref / state，且代际不与既有分支撞号`，期望 `head: null`、实到一个真实 commit id。6 个后端整齐地红成一样，本身就说明问题不在任何一个适配器上，而在被它们共用的那份套件与实现之间。
  - **根因是另一条会话正在改的未提交实现，不是本任务链的交付**：`packages/rxdb-plugin-working-tree/src/commit/branch-commit-rows.ts` 在工作区里是 `M`（97 → 450 行），新增的 `copyCurrentMaterialization()` 让「从当前物化状态建分支」**共享源分支的 HEAD**（`ref.headCommitId = sourceRef.headCommitId`，并附了「不写任何 commit，两条分支指着同一个节点本来就不会互相影响」的理由）；而套件里那条断言写于 `head` 仍为 `null` 的年代。两边都自洽，只是还没对齐。
  - **不改断言、不改实现**：改断言是让门禁迁就一份**正在变**的实现，改实现是替另一条会话决定语义。这条红由那条会话在落地时连同套件一起收口；本任务只如实留下它的形状与归属。
  - **另一半是真绿的**：`working-tree-capture-conformance.spec.ts`（捕获套件）在 6 个后端上零失败，`node scripts/audit/working-tree-suite-callsites.mjs` 报 `6 packages × 2 suites = 12 call sites`。SC-006 的「2 套件」里坏的只有提交套件的这一格。
  - 复跑口径：等 `branch-commit-rows.ts` 那笔改动落地并对齐套件后原样重跑本条命令，全绿即可打勾——**不得**为了打勾去动这条断言。
- [x] T131 按 `specs/001-working-tree-commits/quickstart.md` §3 逐条跑完 3.1–3.10 十个验证场景，把结果记进该文件的执行记录
  ...
  - **九条绿、一条部分红，且红的那一格不是场景原文**：3.1 第 4 步问的是「**既存**分支在 `enable()` 之后都有 ref / state 初始行、代际互不相同」，这一格由 `enable-migration.spec.ts` 的 21 条守着，绿；红的是紧邻的一格——`enable()` **之后新建**的分支（T130 那条，根因在并发会话未提交的 `branch-commit-rows.ts`）。记录里把这条差别写明了，**既不算进通过、也不冒充成 3.1 失败**。
