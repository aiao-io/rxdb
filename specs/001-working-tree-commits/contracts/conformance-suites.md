...
- 迁移**全有或全无**：注入任一分支初始化失败 → 整体回滚，`__rxdb_capability__:workingTree:<版本>`
  那一行**认领行不留下**。判据不是核心的 `RXDB_SYSTEM_SCHEMA_VERSION`——抽包之后那个号归
  `@aiao/rxdb` 自己（今天在 6），本包的启用迁移一步都不动它。要证的始终是「没落地就不认领」。
