...
| 3.7 raw 写被挡在执行前        | `working-tree/raw-bypass-judgment.spec.ts`（49，四步判定 + 解析不出即 fail-closed + 物理表名形态）、`working-tree/raw-write-gate-wiring.spec.ts`（7：启用后同一条语句被拒且执行器一次都没被调用）、`working-tree/observable-gate.spec.ts`（13：`upsertMany`/`deleteByIds` **调用即抛**，调用方手里没有 Observable）、capture 套件 §1.3 四步 + §1.2 行 9/行 10\* | 绿                    |
