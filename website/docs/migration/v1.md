...
第 1~3 项与第 6~8 项改的是接口**成员**或函数**参数**。`requirements/api-baseline/*.json` 只按导出符号的名字和种类
（value / type / both）逐个比对，看不见这一类变化，所以在此手工登记。第 4、5 项是普通的导出移除，
基线已随提交更新（`rxdb-plugin-querycache.json` / `rxdb.json`），一并列在这里方便查阅。

第 1 项与第 5~8 项影响从 `0.0.25` 升级的应用。第 2~4 项动的是 `0.0.25` 之后才出现、还没有发布过的 API：
...
详见 [QueryCache 读引擎拆包](./querycache-plugin.md)。
