...
| 版本与协作   | 无内建版本图                                          | Git 式分支 / 合并 / 撤销重做；工作树与提交历史由 [`@aiao/rxdb-plugin-working-tree`](../plugins/rxdb-plugin-working-tree/README.md) 提供（含 `commit` / `diff` / `restore`，以及带 `requireClean` / `expectedActivationRevision` 前置条件的 `switchBranch`） |
