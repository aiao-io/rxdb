...
/**
 * @fileoverview `/working-tree` 面板的 git 工作流功能用例（Vue 端）。
 *
 * @remarks
 * Angular 版（GitHub Desktop 形态，2026-09-18 二版重构）是参考实现，本拷贝逐条对齐：
 * 顶部分支栏（分支下拉）+ 左侧「变更 / 历史」标签页 + 右栏详情。面板本身**不造数据**——
 * 实体数据在别的页面（/todo）产生，一条实体记录就是工作树里的一个「文件」；分支行是
 ...
 *
 * 框架差异（Vue 应用壳）：`app-sidebar` / `app-header` 是 Angular 壳的选择器，Vue 壳的
 * 等价物是 `#layout-main > aside`（AppSidebar）与 `#layout-topbar`（AppHeader，桌面宽度下
 * 内容为空、只有挂载语义，断言用 toBeAttached）。
 *
...
/** Vue 壳的侧栏（AppSidebar 的 `<aside>`，桌面默认展开 240px）。 */
const APP_SIDEBAR = '#layout-main > aside';
 ...
 * 「编辑文件」这一步发生在别处。
 *
 * **必须走页面内的仓库菜单链接做 SPA 导航，不能用 `page.goto`。** `goto` 是整页刷新：
...
test.describe('Working Tree 页面功能', () => {
  test('保留应用侧栏，工作树只占路由内容区', async ({ page }) => {
    ...
    const appSidebar = page.locator(APP_SIDEBAR);
    ...
    await expect(page.locator('#layout-topbar')).toBeAttached();
    await expect
      .poll(async () => {
  ...
  test('窄屏工作区保留应用菜单入口', async ({ page }) => {
    ...
    await expect(page.locator(APP_SIDEBAR)).toBeVisible();
