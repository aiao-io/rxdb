 ...
 *
 * @remarks
 * `APP-dev-rxdb-vue-fresh-03`：四个 search e2e spec 里**没有一处 scope 断言** ——
 * 三个 demo 页面各自手写的「重建 handle 但保留当前 query」是页面最难的一段逻辑，
 * 却完全没有回归保护。把它迁到 `@aiao/rxdb-plugin-search-vue` 之前先补上，
 * 否则迁移的回归证据是空的。
 *
 * 断言两件事，缺一不可：
 * - 输入框仍是用户打的词（UI 层没被清空）；
 ...
 *
 * 导航和种子就绪判据由 `search-test-api.ts` 统一提供，保证三端 spec 的契约一致。
 */
