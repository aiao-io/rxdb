...
/**
 * @fileoverview `/working-tree` 面板的 a11y 与 SC-005 归档用例（T128，Vue 端）。
 *
 * @remarks
 * 三端各一份，断言逐条对齐 —— 面板的 `data-testid` 与 ARIA 属性在
 * Angular / React / Vue 上完全同名，**任何一端的差异都必须在这里变成红**，
 * 而不是留给用户去发现（contracts/tri-framework-api.md §4）。Angular 版
 * （2026-09-18 二版重构为 GitHub Desktop 形态：顶部分支栏 + 变更/历史标签页 +
 * 右栏详情）是参考实现，本拷贝逐条移植。契约值不变：live region、
 ...
 * overlay 会把节点追加到 body 末尾，Tab 顺序排在面板之后，键盘走不进去），
 ...
 * （白字压 `#6b7280`，≈4.8:1，见 styles.css 的 `.gd-btn-primary:disabled`），
 ...
 *
 * 必须走页面内的仓库菜单链接做 SPA 导航而不是 `page.goto`：整页刷新后 /todo 上的写入
...
test.describe('Working Tree Page A11y', () => {
  ...
  test('面板里的每个控件都能用键盘走到，且焦点可见', async ({ page }) => {
    ...
    // 等它可见再走查——响应式渲染在下一帧，点完立刻 Tab 会打在渲染前的 DOM 上。
