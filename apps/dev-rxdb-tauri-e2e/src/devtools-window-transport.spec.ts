...
describe('开了写入授权的那一跑（US-905 阶段 2）', () => {
  ...
  /**
   * # 这两条曾经是间歇红（2026-09-08 修复）
   *
   * 报告写着 `createDirectory: 'ok'`，盘上却没有那个目录——两句话都是真的，说的却是
   * **两代驱动**：码来自第一代，磁盘上留下的是第二代的手笔。第二代的准备步骤先删
   * `drv-kept` 再建回来，而主窗口紧接着要为 AC#5 刷新一次；刷新换掉了 connector 的
   * session，驱动却只在第一帧上取过一次 session、此后不再更新。刷新（或上报之后那次
   * `app.exit`）一旦落进「删掉了、还没建回来」那一格，create 就被按 session 拒掉，
   * 目录永远停在被删除的状态。`drv-temp` 那一半同形：建了却来不及删。
   *
   * 修法落在源头而不是这两条断言上：第二代驱动本来就没有观察价值（观察者只留第一条
   * 结论），于是回收之后重开的调试窗口不再带注入脚本（`lib.rs` 的
   * `rxdb_devtools_recycle_window`）。一个进程因此只跑一代，而那一代整个跑在
   * `waitForNative()` 的等待里——回收、刷新、退出都插不进它的删与建之间。
   *
   * 放宽断言（改成「目录在或不在都算过」）会把这条链上一次**真实的**写入丢失一起放过去，
   * 而那正是这两条唯一在验的东西。
   */
   ...
   * Rust 就 `app.exit`，报告写出之后就再没有一条通往 provider 的路了。
   */
 ...
 * # 为什么读的是「第一遍」驱动的结论
 *
 * 只有第一遍的前置条件是已知的：那时本进程还没碰过存储。观察者因此只留**第一条**结论
 * （`devtools-probe.ts` 的 `waitForNative`）——在一个被前面改过的世界里，「上一次留下的」
 * 与「本进程刚建的」完全同形，`keptDirSeen` 恒为 `true`。
 *
 * 一个进程本来会跑两遍驱动（探针为 AC#4 把调试窗口关掉再以同 label 重开）。那第二遍现在
 * 不跑了：重开的窗不再带注入脚本，理由见 `lib.rs` 的 `rxdb_devtools_recycle_window`——
 * 它的准备步骤会先删掉 `drv-kept` 再建回来，而主窗口的刷新一旦落进这两步中间，
 * 那个目录就永远停在被删除的状态。
 *
 * # 行数的上下界是怎么来的
 *
 * 探针开着时一个进程会 bootstrap **两次**（AC#5 那次 `location.reload()`），每次追加一行。
 * 而驱动跑在刷新**之前**，所以第一跑经 wire 数到的行数不会超过 1；第二跑起步时盘上
