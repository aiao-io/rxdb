...
async function warmSearch(page: Page, query: string) {
  ...
  await expect.poll(() => page.getByTestId('search-result').count(), { timeout: 15000 }).toBeGreaterThan(0);
}
...
test.describe('Search Refresh', () => {
  ...
  test('reacts to insert/update/delete and keeps query', async ({ page }) => {
    ...
    const query = `refresh-react-${Date.now().toString(36)}`;
    await input.fill(query);
    ...
    // FTS reindex + reactive refresh can exceed 5s under concurrent suite load
