...
export interface DevToolsNativeProbeResult {
   ...
   *
   * @remarks
   * 经 wire 数而不是让 e2e 事后读盘，理由与 {@link cancelledFile} 同源：AC#10 问的是
   * **面板看得到什么**。而这一数必须发生在**进程还活着的时候**——自检一上报 Rust 就
...
export interface DevToolsHandshakeWatcher {
   ...
   * ## 为什么取第一条而不是最新一条
   *
   * 只有第一条的**前置条件是已知的**：那时这个进程还没碰过存储。后来的任何一条都在一个
   * 被前面改过的世界里观察，「重启之后那个目录还在」与「本进程刚把它建出来」在它眼里
   * 完全同形，AC#15 的跨重启比对因此没有判别力。
   *
   * 这一条曾经是**必须**的：探针为 AC#4 把调试窗口关掉再以同 label 重开，而重开的那扇窗
   * 带着同一份注入脚本，于是一个进程里驱动跑两代。那份注入现在停了（`lib.rs` 的
   * `rxdb_devtools_recycle_window` 重开时不再带驱动，理由是第二代会把 e2e 要核对的目录
   * 删在半路），一个进程因此只跑一代。规则留着不动：它写的是「哪一条观察算数」，
   * 而那与「现在恰好只有一条」是两件事。
   *
 ...
 * ## 为什么结论不取刷新后的那一条
 *
 * 驱动的观察全部发生在刷新**之前**（`probeDevToolsWindow` 先 `waitForNative()` 才回收、
 * 才刷新），所以刷新后的观察者手上不该有任何结论。取「刷新前那一份」因此是照实合并，
 * 不是在两份里挑一份。
 *
 * 刷新前一条都没有时就报没有，不拿刷新后掉队的那份充数：「这一代没跑出结论」与「另一代
 * 跑出来了」是两件事，混进同一格之后没人能反推回去。
 */
