...
describe('selectLocalBackend', () => {
  it('顺序即优先级：多个候选同时可用时取第一个', () => {
    const selected = selectLocalBackend([
      candidate('sqlite-tauri', 'demo_desktop'),
      candidate('wa-sqlite', 'demo_web')
    ]);

    expect(selected.adapter).toBe('sqlite-tauri');
    expect(selected.dbName).toBe('demo_desktop');
  });

  it('跳过探针为 false 的候选', () => {
    const selected = selectLocalBackend([
      candidate('sqlite-tauri', 'demo_desktop', { isAvailable: () => false }),
      candidate('wa-sqlite', 'demo_web')
    ]);
  ...
  it('全部不可用时抛错，不回落到任何候选', () => {
    ...
    expect(call).toThrow(/sqlite-tauri, wa-sqlite/);
    expect(create).not.toHaveBeenCalled();
  });
  ...
  it('dbName 重复时拒绝整张表', () => {
    const call = () => selectLocalBackend([candidate('sqlite-tauri', 'test_6'), candidate('wa-sqlite', 'test_6')]);
  ...
  it('表校验先于探针：重复的表连一次探针都不问', () => {
    ...
    expect(() =>
      selectLocalBackend([
        candidate('sqlite-tauri', 'test_6', { isAvailable }),
