...
describe('readDirectoryEntries', () => {
  it('reads every browser batch until an empty batch', async () => {
  ...
  it('rejects when the browser reader fails', async () => {
    const error = new DOMException('denied');
    const reader = {
      readEntries: vi.fn((_success: FileSystemEntriesCallback, failure?: ErrorCallback) => failure?.(error))
    } as unknown as FileSystemDirectoryReader;

    await expect(readDirectoryEntries(reader)).rejects.toBe(error);
  });
});
