...
const handleAddMenu = async () => {
  ...
  if (store.selectedParentId.value) {
    ...
    if (parent) {
      const added = await store.addChild(parent, newTitle.value);
      if (added) newTitle.value = '';
    }
  } else {
    const added = await store.addRoot(newTitle.value);
    if (added) newTitle.value = '';
  }
};
            ...
            <div class="card-body p-4">
              ...
              >
                <div
                  ...
                  v-for="{ menu, level, isExpanded, hasChildren } in store.treeNodes.value"
                  :class="[
                    dragDrop.dragDropState.value.draggedItemId !== menu.id && 'hover:bg-base-200',
                    dragDrop.dragDropState.value.draggedItemId === menu.id && 'cursor-move opacity-50',
                    dragDrop.dragDropState.value.targetItemId === menu.id &&
                    ...
                    dragDrop.highlightedMenuIds.value.has(menu.id) && 'bg-warning/20'
                  ]"
                  :data-dragging="dragDrop.dragDropState.value.draggedItemId === menu.id ? 'true' : 'false'"
                  :data-drop-mode="
                    dragDrop.dragDropState.value.targetItemId === menu.id ? dragDrop.dragDropState.value.dropMode : ''
                  "
                  :data-drop-target="dragDrop.dragDropState.value.targetItemId === menu.id ? 'true' : 'false'"
                  :data-drop-valid="
                    dragDrop.dragDropState.value.targetItemId === menu.id ?
                  ...
                  @dragover="handleDragOver($event, menu)"
                  @dragstart="handleDragStart($event, menu.id)"
                  @drop="handleDrop($event, menu)"
                ...
                >
                  ...
                  <button
                    ...
                    @click="store.toggleExpand(menu.id)"
                  ...
                  <span
                    ...
                    :class="{
                      'bg-yellow-200 font-semibold':
                        ...
                        menu.title.toLowerCase().includes(store.searchKeyword.value.toLowerCase())
                    }"
                    @dblclick="store.startEdit(menu.id)"
                  >
                    {{ menu.title }}
                  </span>
    ...
    >
      <div class="modal-box">
        ...
        <div class="modal-action flex-col gap-2 sm:flex-row">
          ...
          <template v-if="store.deleteImpact.value.childrenCount > 0">
            <button
