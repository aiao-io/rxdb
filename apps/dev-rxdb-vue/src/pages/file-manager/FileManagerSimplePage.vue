...
<template>
  <div class="flex h-full w-full overflow-hidden">
    ...
    <main class="flex h-full min-w-0 flex-1 flex-col overflow-auto">
      <!-- Header -->
        ...
        <div class="mx-auto flex max-w-4xl flex-col gap-3">
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              <h1 class="text-2xl font-bold"> File Manager - Simple </h1>
              <div
      ...
      >
        ...
        <div class="flex-1">
          ...
          <div class="text-sm">
            已存在同名文件：
            <strong>{{ store.pathConflict.value.conflictPath }}</strong>
          </div>
        </div>
        <button
          class="btn btn-ghost btn-sm btn-circle"
          @click="store.clearPathConflict"
        >
          <X :size="16" />
        </button>
      </div>
        ...
        <div class="mx-auto max-w-4xl">
          ...
          >
            <div
              ...
              v-for="{ file, level, isExpanded, hasChildren, isMatched } in store.treeNodes.value"
              :class="[
                dragDrop.dragDropState.value.draggedItemId !== file.id && 'hover:bg-base-200',
                dragDrop.dragDropState.value.draggedItemId === file.id && 'cursor-move opacity-50',
                ...
                dragDrop.highlightedMenuIds.value.has(file.id) && 'bg-warning/20',
            ...
            >
              ...
              <button
                ...
                :disabled="file.type !== 'folder' || !hasChildren"
                @click="store.toggleExpand(file.id)"
                data-testid="file-node-toggle"
              >
                <template v-if="file.type === 'folder' && hasChildren">
                  <ChevronDown
                    v-if="isExpanded"
                    :size="16"
                  />
              ...
              <span class="text-base-content/70">
                ...
                <template v-else>
                  <component
                    :class="{
                      'text-info': getFileIcon('file', file.extension) === 'file-text',
                      'text-success': getFileIcon('file', file.extension) === 'file-code',
                      'text-secondary': getFileIcon('file', file.extension) === 'file-image',
                      'text-error': getFileIcon('file', file.extension) === 'file-video',
                      'text-accent': getFileIcon('file', file.extension) === 'file-audio',
                      'text-warning': getFileIcon('file', file.extension) === 'file-archive',
                      'text-base-content/50': getFileIcon('file', file.extension) === 'file'
                    }"
                    :is="getIconComponent(getFileIcon('file', file.extension))"
                    :size="18"
                  />
                </template>
              </span>
              ...
              <span
                ...
                v-else
                :class="[
                  isMatched && 'rounded bg-yellow-200 px-1',
                  file.type === 'folder' && 'cursor-pointer',
                  store.selectedFolderId.value === file.id && 'text-primary font-semibold'
                ]"
                @click="file.type === 'folder' ? store.selectFolder(file.id) : undefined"
              >
                {{ file.name }}
                <template v-if="file.type === 'file' && file.extension">.{{ file.extension }}</template>
                <span
              ...
              <div class="flex gap-1 opacity-0 group-hover:opacity-100">
                <button
                  ...
                  @click="store.selectFolder(file.id)"
                  aria-label="添加子项"
                ...
                <button
                  ...
                  @click="store.showDeleteDialog(file)"
                  aria-label="删除"
