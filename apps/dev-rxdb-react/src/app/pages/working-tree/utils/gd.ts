 ...
 * 头像颜色就是作者的身份锚点。
 *
 * 与 Angular 参考实现的唯一差异：`gdOpIcon` 在 Angular 里返回 `@lucide/angular`
 * 的组件类，React 端换成 `lucide-react` 的图标组件，选择完全一致（新增 ＋ /
 * 修改 • / 删除 －）。
 */

import type { WorkingTreeDiffEntry } from '@aiao/rxdb-plugin-working-tree';
import { CircleDot, MinusCircle, PlusCircle, type LucideIcon } from 'lucide-react';
...
export const gdOpIcon = (operation: WorkingTreeDiffEntry['operation']): LucideIcon =>
