...
> **2026-08-27 复核**：US-214 已 `Done`。上面那两条 P2 **前提不成立**——250 行的第 6 次请求是空页、
> 空页即短页且恰在末页（P2-1），`run-many` 带多 target 时对缺 target 的 project 是跳过不是报错（P2-2）；
> 两条 P3 已修。本节保留评审当时的原文，逐条处置见[解决记录](#解决记录)。
...
- [x] US-214：四条已处置 —— 2026-08-27，随该故事关闭（`Done`）一并落地。**其中两条 P2 是本评审的前提判断有误，不是漏修**：

  | 条目 | 处置                                                                                                                                                                                                                                                                                                                        |
  | :--- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | P2-1 | **种子仍是 250 行，不需要改**。本评审假定「250 行 = 5 次请求、五页全满」，实测是 **6 次**：offset 翻页收到满页后必然再问一次才知道到底了，第 6 次拿到**空页**。空页 `rows.length(0) < limit`，它本身就是短页，且恰在末页——「短页只出现在末页」有观测路径。已就地更正 AC#4 括号里的算术标注，并记入该故事「落地偏差」第 1 条 |
  | P2-2 | **AC#17 原文那条 `run-many` 实跑全绿，不需要拆**。`nx show project dev-rxdb-http-e2e` 的推断 target 含 `typecheck` / `lint`（缺 `test` / `build`）；而 `run-many` 带多个 target 时，对缺某个 target 的 project 是**跳过**不是报错——「直接报错」只在**所有** project 都没有该 target 时成立                                  |
  | P3-3 | 已修：端口表改为「4200 angular / 4201 react / 4203 vue 与 supabase（两者同端口，历史遗留）；4202 无人使用」                                                                                                                                                                                                                 |
  | P3-4 | 已写死：缺 `Authorization` 放行、带了但格式不对回 `401`，见该故事「落地偏差」第 6 条                                                                                                                                                                                                                                        |

- [x] `status: Resolved`。无修复 PR——US-213 / US-214 的处置均直接落在 `next-1123` 分支上，`pr` 字段留空
