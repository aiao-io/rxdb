...
四轮评审共报 7 条 P1、15 条 P2；已处理（修复或复核后判定不改）的条目按本目录
[README](./README.md) 的约定连同正文一并删除——修法与判据都写在代码注释与红测里，
报告再留一份副本只会与代码漂移。下面留着的 4 条是**判定为债务、暂不动**的。
...
| P2   | `examples/angular-todo` 未迁移 `versionManager`     | 示例升到 0.0.25 即断                              | 示例仓依赖已发布版本，不阻塞主线      |

## 遗留债务明细

### 1. P2：`fillInstant` 模块级共享状态可重入

[`entity.utils.ts:121`](../../packages/rxdb/src/entity/entity.utils.ts#L121) 把本次填充的
...
[`index.ts:17-21`](../../packages/rxdb-plugin-querycache/src/index.ts#L17) 的 5 行 `export *` 把
...
### 4. P2：`examples/angular-todo` 未迁移 `versionManager`

[`examples/angular-todo/package.json:26`](../../examples/angular-todo/package.json#L26) 仍依赖
已发布的 `@aiao/rxdb": "^0.0.24"`（那个版本还带 `versionManager`），所以
[`todo.page.ts:53/295/299`](../../examples/angular-todo/src/app/todo/todo.page.ts#L53) 的
`this.#rxdb.versionManager` 当前不报错。但 `packages/rxdb` 已是 `0.0.25`、`versionManager`
已随 history 插件外移——示例一升版即断。修法照 `modules/angular-todo` 走插件 API。

## 这些条目是怎么查出来的

上面 4 条出自第四轮——那轮是**独立全量评审**，不是针对前几轮修复的复核：10 个发现方向（逐行 diff 扫描 /
