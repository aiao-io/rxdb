...
- **2026-09-19 修复轮**：第四次复核的 8 条里，**2 条 P1 + 2 条 P2 已修并按约定从本报告删除**（见下方「2026-09-19 修复记录」），§4 的 3 条 ✅ 清理项一并落地；另有 1 条 ✅ 复审后翻成 ❌。
- **当前结论**：🔴 **不建议合并**。剩下的 4 条 P1 全是架构级（跨连接能力传播 / 切换事务内 CAS / 物化流水线接公开入口 / 前置条件进最终事务），可由正常跨连接、切换或同步操作触发，测试通过不能替代这些组合时序的回归用例。本报告另有 4 条 P2 与其他待办。
...
- **本轮处理（⏸ Deferred，顺延 4）**：复核成立。不实现的原因：修法要改受信声明的**绑定粒度**（从 adapter 对象改到调用/事务上下文），是 `packages/rxdb` 里 `trusted-write-scope` 的公开面变更，所有声明点与消费点同步改；§4.4 的「受信写标记是自报字符串键」是同一处的更深一层，两条一起决策才不会改两遍。
...
- **修正**：双重捕获（#2）的「origin 被覆盖成 local / push echo」子断言不成立——现有路径内外两次捕获的 origin 都是 local，真实危害是重复条目 + revision 双跳；且因 #1 的存在，外层捕获（裸 ID）根本不与内层条目（`'rxid1'`）折叠，而是各写各的。（双重捕获这条缺陷本身已于 2026-09-19 修复，见文末修复记录第 1 条；此处只保留「origin 被覆盖」这个**子断言不成立**的记录，防再次以那个理由复提。）
...
| [migrate_system_schema.ts:130-143](../../packages/rxdb-adapter-pglite/src/system/migrate_system_schema.ts#L130)                                                                                                                                   | ❌ **判定不改** 零 active 且无 main 行时恢复 UPDATE 静默 no-op，水印照写提交（:266-273）                                 | PLAUSIBLE：状态难以到达（remove_branch 拒删 main），且 pull 路径会先走 resolve 自愈；sqlite 侧注释称该 no-op 为刻意设计。两侧 doc 已补「这里什么都不建」的刻意声明，行为未变。**判定不改，记录防复提**：迁移路径上「没有 main 行就建一行」正是本仓禁止的 fallback 兜底——库里一个分支都没有时，正确的反应是报错而不是替用户捏造一个 main；而该状态本身不可达（`remove_branch` 拒删 main）。刻意声明已写进两侧 doc，不需要再动代码                                                                                                                                                                                                                                                                                                                   |
| [testing.ts:183-191](../../packages/rxdb-adapter-pglite/src/testing.ts#L183)（sqlite-core 侧现居 [`__tests__/test-utils.ts:84-91`](../../packages/rxdb-adapter-sqlite-core/src/__tests__/test-utils.ts#L84)）                                     | ⏸ **顺延（前置：`@aiao/rxdb-test` 边界）** `cleanup_db` 清库后不恢复插件单例行                                           | PLAUSIBLE：代码注释承认是刻意取舍并计划加「由调用方传入初始行」入口（入口未加）；当前仓内无触发路径。**本轮不做**：这是测试工具，且要加的那个入口正好落在 §4.1 三条 ⚠️ 的同一个未决问题上（`@aiao/rxdb-test` 的边界划在哪、`cleanup_db` 是否该改指它）。边界没定就加入口，等于在将来要被合并掉的那份副本上再加一个公开参数                                                                                                                                                                                                                                                                                                                                                                                                                         |
| [capture-hook.ts:459-461](../../packages/rxdb-plugin-working-tree/src/working-tree/capture-hook.ts#L459)                                                                                                                                          | ⏸ **顺延（并入 §4.4 受信写绑定粒度）** raw-write 判定从不传入 intent，step 2 受信放行是死代码                            | CONFIRMED（前瞻性）：判定已重构进 `raw-write-judgment.ts`，上下文类型有可选 `intent` 字段（:57、step 2 在 :683），但唯一生产调用点仍只传 `{capabilityEnabled, domain}`；核心 `gateRawWrite` 与两个适配器调用点整条链均无 intent 槽位，生产路径上 step 2 不可达。**本轮不做**：两个方向现在都不该走——删掉 step 2 会把「受信意图可以放行 raw 写」这条已成文的规则从代码里抹掉；接通它要给 `gateRawWrite` 与两个适配器调用点整条链加 intent 槽位，而意图的承载形式正是 §4.4「受信写标记是自报字符串键」要重定的东西。等那条决策落地后一起接                                                                                                                                                                                                           |
...
| [migrate_system_schema.ts:107-112](../../packages/rxdb-adapter-pglite/src/system/migrate_system_schema.ts#L107)（sqlite-core [`RxDBAdapterSqliteBase.ts:155-157`](../../packages/rxdb-adapter-sqlite-core/src/RxDBAdapterSqliteBase.ts#L155) 同） | ❌ **判定不改** 旧库含 2+ 行 `activated=true` 时迁移抛错，connect 永久失败，无库内恢复路径                               | CONFIRMED（有意的 FR-048 行为）：旧 schema 无约束允许该状态，守卫文档明确「不挑一个留下」；升级即无法打开库。**判定不改，记录防复提**：FR-048 明确要求「不挑一个留下」——两行 `activated=true` 意味着两条分支的工作树都可能有未提交改动，自动选一条等于替用户丢掉另一条的写。这正是「无 fallback 兜底」的适用场景：库处于不可判定状态时拒绝打开，比猜一个答案安全。要补的是**库外恢复工具**（显式指定保留哪条），那是新功能，不是这条缺陷的修复                                                                                                                                                                                                                                                                                                     |
| [commit-graph-guard.ts:209](../../packages/rxdb-plugin-working-tree/src/commit/commit-graph-guard.ts#L209) / [commit-capability.ts:216](../../packages/rxdb-plugin-working-tree/src/commit/commit-capability.ts#L216)                             | 📝 **已补注记，判定为改不了** `corruptedAt` / `enabledAt` 用客户端时钟                                                   | CONFIRMED，但判定为当前改不了：`CURRENT_TIMESTAMP` 在 SQLite 上求值成 `'YYYY-MM-DD HH:MM:SS'`，与本仓日期列的 ISO 存储形态对不上——`sqlTimestampLiteral` 存在的理由正是这个（`branch-materialization.ts:483` 写明）。`corruptedAt` 又是「首次损坏时刻」，列默认值只在 INSERT 时求值而那一行早在建库迁移里就插进去了，用不上。`enabledAt` 走的是手拼 CAS，时刻只能作字面量嵌入。两处已各补一段 TSDoc 写明客户端时钟是有意的、三档边界分别是什么、代价上限是多少（两个值都是诊断/展示值，不参与任何比较）。**统一到数据库时钟要先给仓储层加写 SQL 表达式的能力——架构项，进顺延清单（§5.1）**                                                                                                                                                          |
...
| 判定                          | 位置                                                                                                                                                                                           | 内容与理由                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ⏸                             | [switch-result.utils.ts](../../packages/rxdb-adapter-pglite/src/version/switch-result.utils.ts#L127)                                                                                           | 约 300 行 switch 结果 SQL 脚手架跨 pglite / sqlite-core 复制，注释自承「两家在这一格上必须长得一样」。normalizer 分歧已随 §4.1 的 `normalizeEntity` 修复消失（两端现都调核心 `normalizeUpdateEntity`），但「两端各写一遍、写着写着就分岔」的主问题未动。**本轮不做**：抽公共层要先定它落在哪个包——两个适配器互不依赖，放任一端都会新增跨适配器 import（Nx 图插件会据此连边，见 `MAIN_BRANCH_ID` 那条同样的坑）；与 §4.4 的 `switch_branch` 跨后端对称项、§4.2 的 activeKey 回填序列是同一次抽取，合并做才划算                                                                                                                                                                             |
...
| ⏸    | [capture-runtime.ts:245](../../packages/rxdb-plugin-working-tree/src/working-tree/capture-runtime.ts#L245)     | 每个被捕获变更重读 active-branch token（同事务里上一行刚读过）：一实体一写 4 次查询，M 实体 2M 次冗余查询——捕获热路径。**本轮不做**：性能项，要先有基准才知道改完值多少——`bench-working-tree` 尚未接 CI（见 §5.1 末），没有基准的性能改动无法验收；且本轮刚改过捕获热路径（见「2026-09-19 修复记录」第 7 条），两件事叠在一起会让回归定位不了                                                                                                                                                                                 |
...
| ⏸    | [capture-mount-points.ts:60-96](../../packages/rxdb-plugin-working-tree/src/working-tree/capture-mount-points.ts#L60) | 挂载点注册表第三处编码 core 的原语清单（参数名序列靠源文本 spec 对齐），且运行时从不调用——漏改只会在测试里响。**与上一条同根因**。**本轮不做**：单源化要让 core 把原语清单作为值导出（现在靠源文本 spec 对齐），属 core 公开面变更，与 §4.2 的 `InterceptedBulkWrite` 同一批决策                                                                                                                                                                                                                                                                                                                                                                    |
| ❌   | [active-branch-guard.ts:95](../../packages/rxdb/src/system/active-branch-guard.ts#L95)                                | **判定不改，记录防复提**。core 写字面量 `'ambiguous_active_branch'` 是有意的：core 不能依赖 plugin，而错误码表 `CommitErrorCode` 属于 `rxdb-plugin-working-tree`。反向把码表挪进 core，等于让 core 持有一个插件概念的成员，耦合更差。plugin 侧码表是 const 对象，与 core 的字面量结构兼容；且 [`active-branch-cardinality.spec.ts:169`](../../packages/rxdb-plugin-working-tree/src/__tests__/commit/active-branch-cardinality.spec.ts#L169) 已断言 core 抛出的错误 `code` 等于 `CommitErrorCode.ambiguous_active_branch`——任意一侧改动都会红，「漂移无人知」的前提不成立。原判语「单源化成本极低」把成本算漏了：便宜的是改字符串，贵的是改依赖方向 |
| ⏸    | [switch_branch.ts:78-100](../../packages/rxdb-adapter-sqlite-core/src/version/switch_branch.ts#L78)                   | 分支翻转修复与 activeKey 回填按后端各写一遍（pglite 同路径 :97-121），且两端修复同一根因（多语句 RETURNING 丢行）用了不同机制——与 §4.1 的 switch-result 脚手架复制同属跨后端对称问题，应合并处理。**本轮不做**：理由同 §4.1 那条（公共层落在哪个包未定），且两端用了**不同机制**修同一根因，合并前要先判哪一种是对的——这是一次行为决策，不是纯搬运                                                                                                                                                                                                                                                                                                  |
...
上轮列在这里的另两条（`merge_branch('normal')` 双重捕获、diff 分页切断事务）**已于 2026-09-19 修复**，见下方「修复记录」；两条都不是架构级，之前被归到这一节是判错了规模。本节余下的顺延项还有：§4 各表里打 ⏸ 的条目（复用/简化/效率/根因深度，各自的前置条件写在行内）、§3 打 ⏸ 的四条次要项，以及 `bench-working-tree` 接 CI（T132 已于 2026-09-18 按 T109 复冻基线跑过 `✓ PASS`，剩 CI 接线未做）——**§4.3 的五条性能项都卡在它后面**：没有 CI 基准，性能改动没有验收口径。完整清单见 `next-0912-branch-review-max.md` §6.1。

### 5.2 尚未排期

§3 与 §4 的每一行现在都带判定，本节只留**跨行的前置决策**——这些问题不定，被它们挡住的行就没法开工：

1. **跨连接是否在威胁模型内**（一个库同时被多个连接写）。挡着：顺延 1、§3 的 `allocateBranchGeneration` 无 CAS
2. **core ↔ plugin 的公开面边界**：core 是否愿意把 `InterceptedBulkWrite`、捕获原语清单这类内部形状作为公开面被插件钉住。挡着：§4.2 的 bulk-write-gate 重声明、§4.4 的挂载点清单单源
3. **适配器公开面能否扩**（物理表名解析、`switchBranch` 入参形状、跨后端公共层落在哪个包）。挡着：顺延 3、§4.1 的 switch-result 脚手架、§4.4 的表名解析与 switch_branch 对称项、§4.2 的 activeKey 回填序列
4. **第三方适配器是否在威胁模型内**。挡着：§4.4 打 ⚠️ 的受信写自报键、`'*active*'` 哨兵；连带 §3 的 raw intent 死代码（意图的承载形式要先定）
5. **词法扫描门禁是否改用 AST**：二次评审后这一问**变窄了**——模板插值那条已用按段扫描（而不是 AST）修掉，仍悬而未决的只剩 §3 那条正则字面量误判
6. **`@aiao/rxdb-test` 的边界**。挡着：§4.1 的三条 ⚠️ 测试工具复用、§3 的 `cleanup_db` 不恢复单例行
7. **`bench-working-tree` 接 CI**。挡着：§4.3 的五条性能项（没有基准就没有验收口径）

§4.5 的 4 处定向 lint 抑制维持 ❌ 不改（单行、有注释理由、lint 仍报零警告），不在上述任何一条的下游。

## 2026-09-19 修复记录

本轮按「值得修就修，修完删条目」的约定处理了 8 项；条目已从上文删除，**修法与判据写在代码注释 / TSDoc 里**，这里只留索引，便于回查是哪一条被删了。

| #   | 原条目                                                           | 落点                                                                                                                                                                                                     | 红测试                                                                                                                                                                  |
| --- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | [P1] `normal` 合并在同一事务内重复捕获（第三、四次复核各记一条） | [`capture-hook.ts`](../../packages/rxdb-plugin-working-tree/src/working-tree/capture-hook.ts) 新增 `CONSUMED_CHANGE_IDS`（按事务分格的「已认领变更行 id 集合」）                                         | `capture-runtime-mount-points.spec.ts` +2：一条钉「同一批写只判一遍」，一条钉**反命题**——排除的是被认领的那几行、不是抬高水位线，合并之前同一事务里的业务写照旧进工作树 |
| 2   | [P1] Angular 行级 Discard 实际丢弃整个工作树                     | [`working-tree.page.ts`](../../apps/dev-rxdb-angular/src/app/pages/working-tree/working-tree.page.ts)：菜单项改名 **Discard All Changes** + 不可撤销确认框；顺手删掉模板里绑了个不存在输出的 `(discard)` | `working-tree.spec.ts` 的 e2e 改成**两条**未提交改动——只留一条时「丢这一行」与「丢全部」结果一样，这条缺陷一条用例都碰不到                                              |
| 3   | [P2] 共享查询状态会被迟到请求覆盖                                | [`working-tree-commands.ts`](../../packages/rxdb-plugin-working-tree/src/working-tree/working-tree-commands.ts) 的 `createLatestOnlyPatch`（按槽位的请求代际守卫）                                       | +4                                                                                                                                                                      |
| 4   | [P2] transaction 粒度 diff 分页切断原子组                        | [`diff.ts`](../../packages/rxdb-plugin-working-tree/src/working-tree/diff.ts) 的 `assertPageableGranularity`：`granularity:'transaction'` 配 `limit`/`cursor` 直接拒绝，不静默切                         | +4；契约文档 `specs/001-working-tree-commits/contracts/core-api.md` 同步                                                                                                |
| 5   | §4.1 ✅ SQL 引号工具 3 份副本、NUL 策略已分歧                    | pglite / sqlite-core 的 `quoteIdentifier` / `quoteLiteral` / `quote_sql_identifier` 改指 core 的 `sql-literal` —— 分歧消失在「只有一份实现」上，不是靠把三份对齐                                         | 既有用例                                                                                                                                                                |
| 6   | §4.2 ✅ `columnOf` 五份副本                                      | 新增 [`entity-column.ts`](../../packages/rxdb-plugin-working-tree/src/entity-column.ts) 的 `createColumnOf` 工厂，五处改指                                                                               | 既有用例                                                                                                                                                                |
| 7   | §4.2 ✅ `nextParents` 与 `nextFrontier` 字节级相同               | `nextFrontier` 从 `list-commits.ts` 导出，守卫改为导入                                                                                                                                                   | 既有用例                                                                                                                                                                |

**验证**：`pnpm nx run-many -t lint typecheck test -p rxdb-plugin-working-tree rxdb-plugin-working-tree-{angular,react,vue} rxdb-adapter-{pglite,sqlite-core} rxdb-plugin-history --skip-nx-cache` 全绿（7 projects / 31 tasks）；`pnpm nx e2e dev-rxdb-angular-e2e -- src/working-tree.spec.ts` 9 passed。

**本轮判定为不改的两条**（理由写在对应行里，防复提）：§4.4 的 `ambiguous_active_branch` 错误码单源化（修法要倒转 core→plugin 依赖方向，而现有测试已钉死两侧，漂移不可能无人知）、§3 的零 active 恢复 UPDATE 与 2+ active 行迁移抛错（两条都是「无 fallback 兜底」的正例，不是缺陷）。
