...
| code-scanning  | `CS-XXX`        | 与 GitHub code scanning 告警 `number` 一一对应；工作集条目，GitHub 关闭后归档进该目录 README 并删除文件 | `code-scanning/`    |
...
本地文件是**工作集条目，不是历史档案**：GitHub 上告警变为 `fixed` / `dismissed` 后，
在下一次增量同步时把处置结论归档进 [code-scanning/README.md](./code-scanning/README.md#归档) 并删除文件，
不在目录里保留已关闭告警的镜像。
...
实现过程中产生的**演进叙述**（`已于 X 日关闭`、`原判定留档`、`~~删除线~~`、`落地偏差`、
`实现阶段的结论`、`后续变更`、`复核/改判记录`）**不得进入需求正文**。处置分三种：

- **纯过程（怎么走到这儿的）**：就地删除，git 历史就是记录。
- **对后来者有价值的教训**（如「某条 AC 绿得不作数」的根因、被推翻的设计前提）：落进
  `website/docs/migration/` 或对应实现的注释，需求正文只留一句当前结论。
- **仍生效的约束 / 缺口**：写成当前事实——不含日期、不写「已解除/已取消」的对比叙述。

派生视图（`status-overview.md` / `roadmap.md` / `capability-matrix.md`）尤其如此：它们回答「现在」，
不是「曾经」。**「已完成 / 已移出 / 已解除」的条目从正文移除**，被别的文档通过锚点引用的段落保留标题、
只把正文收敛成当前结论。
