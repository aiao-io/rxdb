...
| #4       | `file/protocol.rs` 的 `rejects_every_path_shape_that_could_escape_the_storage_root`（26 种形状：`..` / 绝对路径 / 盘符 / 反斜杠 / NUL / 控制字符 / 尾点尾空格 / Windows 保留名 / 超长段）+ `file/mod.rs` 的 `refuses_paths_that_resolve_outside_the_storage_root`（拼完再比根前缀，挡根内符号链接）与 `never_leaks_the_physical_root_in_a_response`。越界路径在**解析层**即被拒，根本走不到任何 fs 调用；`capabilities/default.json` 全程零改动，只有 `core:*` 窗口权限，无 `fs`、无 `shell`                                        |
