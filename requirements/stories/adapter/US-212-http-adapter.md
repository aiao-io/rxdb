...
type FetchMetadataResult =
  ...
  | { rows: QueryCacheEntityMetadata[]; nextPageToken?: string }; // 形态 2：token 终止
...
**任一条不满足的服务端 MUST 用 token 形态。** 这是本包文档的显式要求，不是建议：适配器无法在客户端侧检测短页截断，只能靠这条契约把责任划清。

**fail-fast（不是 fallback，是拒绝）：**

- **换形态**：同一次查询中途首页数组、次页对象（或反之）→ 抛错。混用会让两条终止判据互相盖掉，正是本故事要防的静默截断。
- **token 不推进**：`nextPageToken` 与上一页相同 → 抛错，不进死循环。
- **连续空页**：`rows` 为空却仍带 `nextPageToken` 的页连续出现 `maxEmptyPages` 次（默认 3）→ 抛错。远端在空转，继续翻只会拉长故障暴露时间。
- **总页数触顶**：超过 `maxPages`（默认 1000）→ 抛错，**不得**返回已拿到的部分。返回部分结果就是把静默截断搬进了客户端。

空页计数器的精确语义（否则它会和「空 `rows` 仍带 `nextPageToken` 时必须继续翻」互相打架，见 AC#6）：

- **只在 token 形态生效**。offset 形态下空页即短页，本来就是正常终止条件，不计数。
- **连续计数，非空即清零**。空一页不是故障，空 `maxEmptyPages` 页才是。所以「空 `rows` + 有 `nextPageToken` → 继续翻」与「连续空页触顶 → 抛错」不矛盾：前者描述第 1…N−1 次，后者描述第 N 次。
...
| 11  | 同上                                                   | 检查 `pullChangesBatch`                                                                                                                                | 不实现（调用点 [`pull-batch.ts`](../../../packages/rxdb-plugin-history/src/pull-batch.ts) 做特性探测，回落到同样 throw 的 `pullChanges`）；若实现则必须 throw，不得返回 `[]`                                                                                                                                                                                                                                           | ✅   |
...
`pullChangesBatch` 是 `RxDBAdapterRemoteBase` 上的 **optional** 成员，调用点 [`pull-batch.ts`](../../../packages/rxdb-plugin-history/src/pull-batch.ts) 做 `if (remoteAdapter.pullChangesBatch)` 特性探测。因此**不实现**才是符合约定的做法（自动回落到同样 throw 的 `pullChanges`）；真正的不变量是「不得返回空数组」，不是「必须实现成 throw」。
