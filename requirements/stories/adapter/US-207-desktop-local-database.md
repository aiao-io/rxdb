...
`packages/rxdb-adapter-electron/src/__tests__/setup.spec.ts` 把 `@aiao/rxdb-adapter-sqlite-core/testing`
...
| 9   | `desktop-sqlite-client.ts` 的 `negotiateProtocolVersion` 在**任何有副作用的请求之前**发一条无参数的 `handshake`，用 `parseDesktopHostHandshakeResult` 比对 `DESKTOP_HOST_PROTOCOL_VERSION`，不等即抛 `protocol_violation`（`open` 应答上那道 `parseDesktopHostOpenResult` 保留为第二道防线，也是老 renderer 唯一的检查点）；`rxdb-adapter-electron` 的 `electron-sqlite-client.spec.ts` 三条用例分别撞版本不符、握手排序（`kinds` 恰为 `['handshake', 'open']`）与「老到不认识握手的 host」，每条都断言 `openSessionCount === 0` 且工作区目录为空——「不建库」由此是断言而不是说法；同包 `electron-sqlite-host.spec.ts` 从 host 侧断言握手既不开会话也不 consult `resolveDatabasePath` |
...
| 3    | 包边界重整：E1～E7                     | 见各任务判据；与 [US-210 T1～T7](./US-210-tauri-sqlite-local-database.md#tauri-包化) 同批做，两侧共用一次 `ADAPTER_NAME` 改名。E1～E5、E7 已交付，E6 的迁移文档已写完，只剩 `npm deprecate` 一步（对外不可逆，需人工确认）      | ⚠️ 进行中 |
| 4    | Web 回落：E8～E11                      | 见各任务判据；「选择器是否公开 API」已判定为**否**（2026-08-18，留在应用里）。E8 的选择器与 E9 的同名断言已落在 `dev-rxdb-tauri` 的应用代码里；欠 `dev-rxdb-electron` 半边（要求合并两卡，见 E8 备注）、E9 的页面展示、E10、E11 | ⚠️ 进行中 |
...
> **进度（2026-08-18）**：E1～E5 与 E7 已落地，`packages/rxdb-adapter-desktop` 这个目录与
> `@aiao/rxdb-adapter-desktop` 这个 workspace 依赖在仓库里都不存在了，三条发布性质也已在两个包的
> 真 tarball 上跑通。E6 被拆成两半执行：**迁移文档已交付**（`website/docs/migration/desktop-split.md`），
> **`npm deprecate` 未执行**——它改的是公共 registry 上已发布包的元数据，不可逆，需人工确认。
> 下面的现状描述保留为拆包前的原文，读作「为什么要拆」。
...
> **进度（2026-08-18）**：选择器本体已落 —— `selectLocalBackend()` / `LocalBackendCandidate` /
> 两个错误类（`apps/dev-rxdb-tauri/src/app/local-backend.ts`，10 条单测），候选表在同目录的
> `setup_rxdb.ts`；E9 的「同名候选直接拒绝构造」也已经是断言而不只是约定。
> **它是应用代码，不是框架 API** —— 曾短暂进过 `@aiao/rxdb` 的公开导出，同日判定撤回，
> 理由见下方「为什么不做成公开 API」。
> 欠的是 `dev-rxdb-electron` 那一半（见 E8 行下的说明）、E9 的页面展示、E10、E11。
...
| #     | 任务                                                                                                                                                                                           | 完成判据                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| E8 ⚠️ | 判定逻辑从内联 `? :` 里拆出来：**纯函数**选择器，输入是应用给的候选表（`{ adapter, create, isAvailable }` 之类），输出成对的名字 + 工厂；不内建候选，`isTauriRuntime()` 这类探针随各运行时包走 | 选择器**不依赖任何适配器包**（内建候选表会让它反向依赖全部适配器）；两个 demo 改用它，`dev-rxdb-electron` 补齐今天缺的那一半；探针可注入，单测不靠真实 `globalThis`。**已达成**：`apps/dev-rxdb-tauri/src/app/local-backend.ts` 落 `selectLocalBackend()` + `LocalBackendCandidate`；10 条单测一律注入假 `runtime`，不碰真实 `globalThis`；`dev-rxdb-tauri` 已改用它（`setup_rxdb.ts` 只剩一张候选表，`app.config.ts` 两处调用点共用同一次判定）。**判据变更**：原判据写的是「上移到共享层」并以「选择器所在包不依赖适配器包」验收，2026-08-18 判定不上移（理由见下方「为什么不做成公开 API」），该条降级为对函数本身的约束——它只 import `@aiao/rxdb` 的 `RxDBError` / `RxDBAdapterName`，适配器由候选表带入。**未达成**：`dev-rxdb-electron` 那一半，理由见下方「Electron 半边为什么还没动」 |
| E9 ⚠️ | 后端身份可观测：选中结果暴露给应用（连接状态旁边就能读到「现在跑在哪个后端」），且候选表禁止复用同一个 `dbName`                                                                                | demo 已有的 `desktop_demo` / `test_6` 分名不是巧合，要变成选择器的**断言**：同名候选直接拒绝构造；页面上能看出当前后端，不靠猜。**已达成（断言那半）**：`dbName` 与 `adapter` 重复都由 `RxDBLocalBackendTableError` 在问探针**之前**拒掉。写这条时发现「分名不是巧合」只对 Electron 成立 —— `dev-rxdb-tauri` 的两个候选原本**都叫 `test_6`**，桌面那份因此改名 `desktop_demo`（与 Electron 对齐），README 6 处与 e2e 的 `DATABASE_FILE` 一并更新。**未达成**：页面上读出当前后端                                                                                                                                                                                                                                                                                                              |
...
| E11   | 候选走动态 `import()`，桌面分支不进 web bundle                                                                                                                                                 | 今天 `setup_rxdb.ts` 是**静态** import 两条分支，浏览器预览的 bundle 里带着 Tauri transport；US-505 AC#10「Tauri 传输客户端代码不进浏览器 bundle」在包一级成立、在应用一级被绕开。改后需有产物断言，否则这条只是口头承诺。**阻塞已解除（2026-08-18）**：三端 provider 统一收 `RxDBSource = RxDB \| Promise<RxDB> \| (() => RxDB \| Promise<RxDB>)`，`await import()` 可以直接交给 `provideRxDB` / `<RxDBProvider db>`，不必再自建异步 initializer + token；Angular 侧的 initializer 由 `provideRxDB` 自带。剩下的是候选表内部的事：把 `create` 改成 async、加产物断言                                                                                                                                                                                                                         |
...
`dev-rxdb-tauri` 是**一个** RxDB 按运行时二选一；`dev-rxdb-electron` 是**两个** RxDB 并存 ——
...
也就是说 E8 在 Electron 侧不是接线改动，它**要求那两张卡片合并成一张**（当前后端 + 连接状态），
而两卡并存是 `desktop-database.service.ts` 里写明的教学意图（"两张卡片摆在一起才看得出
US-207 换掉的到底是什么"）。E9 的「页面上能看出当前后端」正是它的替代物。

连带影响：`home.page.ts` / `home.page.html` / `storage.page.ts`，以及
`dev-rxdb-electron-e2e` 的 `desktop-persistence.spec.ts` + `storage-persistence.spec.ts`
（合计约 450 行，断言的正是两实例形态）。**这两个 e2e 需要真实 Electron 产物才跑得起来**，
改完无法在本地自证，所以没有顺手做掉 —— 等确认「合并成一张卡」这个 demo 形态变更之后再动。
