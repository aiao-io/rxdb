...
| 2    | 打包与发布门禁：AC#8 +「三条发布性质」 | release 触发的三平台 workflow 绿（见「三平台打包 CI」）。workflow 与审计脚本已落地，本机 macOS 全绿；**三平台首轮已确认**（2026-08-17 [run 32075648469](https://github.com/aiao-io/rxdb/actions/runs/32075648469) 与 2026-08-19 [run 32311812029](https://github.com/aiao-io/rxdb/actions/runs/32311812029)：`electron-smoke` × 3 + `tauri-smoke` × 3 + `adapter-consumer` + `gate` 全 success） | ✅ 已交付 |
...
| 4    | Web 回落：E8～E11                      | 见各任务判据；「选择器是否公开 API」已判定为**否**（2026-08-18，留在应用里）。E8～E11 全部交付：两个 demo 共用同一形态的选择器，Electron 侧两卡已合并成一张，E11 的产物断言落成 `scripts/audit/desktop-lazy-backend.mjs` 并进了 CI                                                                                                                                                               | ✅ 已交付 |
...
> **已交付（2026-08-18）**：选择器本体是 `selectLocalBackend()` / `LocalBackendCandidate` /
