...
| 4    | Web 回落：E8～E11                      | 见各任务判据；「选择器是否公开 API」在本阶段 plan 时定。E8 的选择器与 E9 的同名断言已落在 `@aiao/rxdb`，`dev-rxdb-tauri` 已改用；欠 `dev-rxdb-electron` 半边（要求合并两卡，见 E8 备注）、E9 的页面展示、E10、E11          | ⚠️ 进行中 |
...
> **进度（2026-08-18）**：选择器本体已落 —— `@aiao/rxdb` 导出 `selectLocalBackend()` /
> `LocalBackendCandidate` / 两个错误类（`packages/rxdb/src/local-backend.ts`，10 条单测），
> `dev-rxdb-tauri` 已经改用它，E9 的「同名候选直接拒绝构造」也已经是断言而不只是约定。
...
| #     | 任务                                                                                                                                                                                            | 完成判据                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| E8 ⚠️ | 判定逻辑从 demo 上移：共享层提供**纯函数**选择器，输入是应用给的候选表（`{ adapter, create, isAvailable }` 之类），输出成对的名字 + 工厂；不内建候选，`isTauriRuntime()` 这类探针随各运行时包走 | 选择器所在包**不依赖任何适配器包**（内建候选表会让它反向依赖全部适配器）；两个 demo 改用它，`dev-rxdb-electron` 补齐今天缺的那一半；探针可注入，单测不靠真实 `globalThis`。**已达成**：`packages/rxdb/src/local-backend.ts` 落 `selectLocalBackend()` + `LocalBackendCandidate`；选在核心包是因为它已经拥有 `RxDBAdapterName`、且是全部适配器的**下游**（放进任一适配器包都会成环）；10 条单测一律注入假 `runtime`，不碰真实 `globalThis`；`dev-rxdb-tauri` 已改用它（`setup_rxdb.ts` 只剩一张候选表，`app.config.ts` 两处调用点共用同一次判定）。**未达成**：`dev-rxdb-electron` 那一半，理由见下方「Electron 半边为什么还没动」 |
...
| E11   | 候选走动态 `import()`，桌面分支不进 web bundle                                                                                                                                                  | 今天 `setup_rxdb.ts` 是**静态** import 两条分支，浏览器预览的 bundle 里带着 Tauri transport；US-505 AC#10「Tauri 传输客户端代码不进浏览器 bundle」在包一级成立、在应用一级被绕开。改后需有产物断言，否则这条只是口头承诺。**已知阻塞**：`provideRxDB` 收的是**同步**工厂 `() => RxDB`，而两个 demo 的建库函数还要 `inject(PLATFORM_ID)`；把 `await import()` 塞进 `create` 需要先把这层接线改成异步 initializer + token，不是候选表内部能解决的                                                                                                                                                                                   |
...
已落的是**形态**：`selectLocalBackend` 进了 `@aiao/rxdb` 的公开导出（api-baseline 已登记），
但框架层没动 —— `provideRxDB` 那一层三端各写一遍的事还没开始，所以三框架对称铁律尚未被触发。
