...
| 3    | 包边界重整：E1～E7                     | 见各任务判据；与 [US-210 T1～T7](./US-210-tauri-sqlite-local-database.md#tauri-包化) 同批做，两侧共用一次 `ADAPTER_NAME` 改名。E1～E7 全部交付；E6 的 `npm deprecate` 于 2026-08-18 判定**不做**（旧包保留可更新），判据已随之收窄                                                                                                                                                                                       | ✅ 已交付 |
...
> **进度（2026-08-18）**：E1～E7 全部落地，`packages/rxdb-adapter-desktop` 这个目录与
> `@aiao/rxdb-adapter-desktop` 这个 workspace 依赖在仓库里都不存在了，三条发布性质也已在两个包的
> 真 tarball 上跑通，迁移文档已交付（`website/docs/migration/desktop-split.md`）。
> **`npm deprecate` 判定不做**：旧包 `@aiao/rxdb-adapter-desktop@0.0.25` 保留在 registry 上，
> 未来仍可继续发版；打 `deprecated` 标记会与「这个包还会更新」自相矛盾，因此迁移路径只靠文档给，
> 不动 registry 元数据。本阶段不再有挂起的对外动作。
