...
- **US-905b：真实 provider 与三平台证据。** 依赖 US-905a、US-904c、US-210、US-505；接入真实
  SQLite/native files，完成诊断、Settings、重启 E2E 和 macOS / Windows / Linux smoke。

固定关系为 **US-904b → US-905a**，以及
**US-904c + US-905a + US-210 + US-505 → US-905b**。不得等待全部 native host 完成后才开始
...
- 调试窗口通过 US-210 检查当前 Tauri SQLite 的逻辑数据库、实体与事件，通过 US-505 浏览
  插件专用原生文件根
- 文件页支持与 US-904b 对称的刷新、目录导航、上传、下载、新建目录和删除；Storage 页消费
  US-904b 的完整分页诊断快照。snapshot 未完成或已失效时不展示确定性诊断
...
| ------------------------------- | ---------------------- | --------------------- | ---------------------------------------------- |
| wa-sqlite / `OPFSCoopSyncVFS`   | 保持 Web 能力          | browser OPFS provider | 下载 unsupported；清理按 browser provider 能力 |
| wa-sqlite / `IDBBatchAtomicVFS` | 保持 Web 能力          | unsupported           | 下载 unsupported；清理按 IDB provider 能力     |
| wa-sqlite / `unavailable`       | 结构化 unavailable     | unsupported           | 下载与清理均 unsupported，不创建 fallback      |
| US-210 Tauri SQLite             | 本故事                 | 不适用                | 下载 unsupported；清理按 provider 能力启用     |
| US-505 Tauri native files       | metadata 本故事        | 本故事                | 数据库下载 unsupported；文件操作限插件专用根   |
...
| 契约范围                                              | 去向                                                               |
| ----------------------------------------------------- | ------------------------------------------------------------------ |
| Tauri dev window、v2 transport、身份与 release 隔离  | [US-905a](./US-905a-tauri-devtools-window-transport.md)            |
...
- Tauri transport 复用 US-904b 的 breaking v2 与“宽外层、严内层”解析；外层必须能返回 `protocol_unsupported`，
...
- v2 固定采用 canonical UUID v4 session、1～128 ASCII request/transfer ID、每 session 32 个请求/
  2 个传输、256 KiB chunk、100/500 分页和 15 秒超时；provider 必须声明 `maxTransferBytes`。
  Tauri 不放宽限制，也不新增平台私有错误码
- 诊断快照最多从头重试 3 次且总计不超过 15 秒，耗尽返回 `snapshot_busy`；所有数据库下载均返回
  `export_unsupported`

## 依赖与排期

- [US-904b](./US-904b-devtools-shared-protocol-panel.md)：US-905a 的唯一功能前置，提供平台无关 panel、
  v2 transport、provider 联合、session 身份、分页诊断和共享 fixture
- [US-904c](./US-904c-electron-native-devtools-integration.md)：US-905b 复用其 native provider fixture；
  不阻塞 US-905a
...
**US-904c + US-905a + US-505 → US-905b**。US-905a 与 US-210 / US-505 可并行，只有
