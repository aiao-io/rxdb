...
1. **e2e 失败诊断缺口**：[`playwright.config.ts`](../../../apps/dev-rxdb-angular-e2e/playwright.config.ts) 已开
...
5. **rrweb 能力边界**：MIT 许可、v2.x 活跃维护。其回放是「录制 DOM 的重新渲染」，**不重执行应用代码**——
   回放提供观察级还原；交互调试必须在 restoreSession 后的活应用里进行。执行级确定性复现（浏览器引擎级录制）
...
| B    | ⬜   | 事件流与 working-tree commit 的关联标记 + 失败时刻数据状态还原     | AC#1～7、8～10   | US-307 `Done`（restoreSession 能力可交付）                                                                |
...
- 阶段 B：每次 working-tree commit 向事件流写入携带 commit revision 的标记事件；回放页从时间轴选点 → restoreSession 恢复数据 → 活应用承载交互调试。
...
|   8 |  B   | 录制期间 working-tree 产生多次 commit          | 检查事件流                                                         | 每次 commit 在事件流中留下携带该 commit revision 的标记事件，顺序与 commit 一致；标记来自真实 commit 生命周期 |  ⬜  |
|   9 |  B   | 回放时间轴显示 commit 标记                     | 在回放页选择某一 commit                                            | 应用数据恢复到该 commit 状态（restoreSession 语义），断言与 commit 内容一致                                   |  ⬜  |
|  10 |  B   | 数据已恢复到失败时刻状态                       | 在活应用上交互                                                     | 失败场景的数据前置条件成立（复现 bug 属调试动作，不作为 AC 判据）                                             |  ⬜  |
...
- **commit 关联挂点**：rrweb 自定义事件（`EventType.Custom`）携带 commit revision，写入时机挂 commit 生命周期。
...
- **回放与还原分工**：rrweb 回放 = 观察级；restoreSession = 状态级。本故事的调试闭环是
...
| B    | `apps/dev-rxdb-angular/`（或回放页）                       | 时间轴选点 → restoreSession → 活应用      |
