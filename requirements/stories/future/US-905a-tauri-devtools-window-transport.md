...
- [x] Estimable: bootstrap、身份、fixture、VFS 映射与生命周期边界已固定
...
- wa-sqlite demo 按运行时真实选中的 `OPFSCoopSyncVFS`、`IDBBatchAtomicVFS` 或 `unavailable`
  映射 provider，不能根据 adapter 名、URL 或平台猜测
...
| 2   | 真实主窗口与调试窗口已打开                          | 用共享 fake providers 执行查询、事件、分页和文件操作 | 状态与 US-904b fixture 一致；Tauri 只适配 transport，不复制 panel、provider 类型、错误码或状态机                                          | ⬜   |
| 3   | 非调试窗口、错误 sender 或错误 label 构造合法 v2 消息 | 通过 transport 发送                                | WebView、transport 与 Rust 边界均拒绝；仅凭可复用 label 不能取得 session                                                                  | ⬜   |
| 4   | session A 有订阅、请求和未完成传输                  | 关闭窗口，以同 label 重开 B 并投递 A 消息          | A 的资源释放，B 获得新 UUID v4 session 并拒绝全部旧身份、事件、响应与 chunk                                                               | ⬜   |
| 5   | 主窗口刷新、transport 断开或应用退出                | 观察 connector/provider 生命周期                   | 订阅、计时器、请求和传输均取消；fake provider owner 释放，不留下可被重连复用的 host session                                               | ⬜   |
| 6   | wa-sqlite 分别实际选择 OPFS、IDB、unavailable       | 打开调试窗口查看 provider                          | 分别声明 browser OPFS、IDB 能力或 unavailable；IDB/unavailable 不展示 OPFS 文件或数据库下载                                               | ⬜   |
| 7   | v1/v2 不兼容、限额超出、超时与连续失效 fixture      | 通过 Tauri transport 执行                          | 结果与 US-904b 的错误、清理和 `snapshot_busy` 完全一致，不增加平台错误码或 fallback                                                       | ⬜   |
...
- 共享面板通过正式 workspace dependency 消费 `packages/rxdb-devtools-panel/`，不复制源码或用
  tsconfig path 绕过依赖。
- v2 的标识、并发、分块、分页、超时与错误码全部继承 US-904b。
