...
- [x] Estimable: 协议控制面、provider 数据面、面板抽取、Chrome 迁移已分别落入 US-904b1/b2/b3/b4
- [ ] Small: 不成立，已拆成 US-904b1 / US-904b2 / US-904b3 / US-904b4
- [x] Testable: 四条子故事各自持有协议、数据面、library 边界和真实 Chrome relay 的自动验收
-->

# 用户故事：DevTools 共享 v2 协议与面板（契约父故事）

> **本文件不直接交付。** 它是四条子故事共享的版本、安全、数据面和发布边界；四条全部
> `Done` 后才把本父故事置 `Done`。
>
> | 子故事                                                  | 交付                                                    |
> | ------------------------------------------------------- | ------------------------------------------------------- |
...
> | [US-904b3](./US-904b3-devtools-shared-panel-library.md) | 行为中性的私有 Angular 面板抽取与 transport token       |
> | [US-904b4](./US-904b4-devtools-chrome-v2-migration.md)  | Chrome 四段 relay、v2 切换、迁移桥与浏览器回归          |
...
3. **US-904b3：共享面板 library 抽取。** 在**现有 v1 wire 上**行为中性地抽出私有 Angular library 和
   transport token；不依赖 b1/b2，可与其并行。
4. **US-904b4：Chrome v2 迁移。** 消费 b1/b2/b3，把 token 背后的实现切到 v2，并完成真实 extension
   relay 与浏览器回归。

固定关系为 **(US-904b1 → US-904b2) ∥ US-904b3**，二者汇合于 **US-904b4**。整条链与
...
拆出 b3 的理由：面板抽取是纯结构改动，与协议设计没有真实依赖，捆在一起会让 UI 搬迁被协议评审阻塞，
也会让「行为中性」和「行为收敛」两类 diff 混在同一次审查里。
...
- v1 bridge 至少保留一个 fixed release 次版本；删除前同步迁移文档、扩展最低 connector 版本和发布说明。
  兼容形态在 US-904b4 的 plan 阶段二选一并写明理由：**完整 facade**（旧 connector 继续可用，但要长期
  维护两套语义映射）或**版本闸门**（只回一条“connector 版本过低，请升级到 ≥ X”并停止会话，维护成本低
  但直接打断旧应用的调试）。选 facade 必须同时给出它维护到哪个版本，不允许无限期存在
...
| 私有共享面板 library、transport token 与发布隔离        | [US-904b3](./US-904b3-devtools-shared-panel-library.md) |
| Chrome 四段 relay、v2 切换、v1 bridge 与浏览器回归      | [US-904b4](./US-904b4-devtools-chrome-v2-migration.md)  |
...
| `apps/rxdb-devtools-extension/src/`    | b3 / b4  | b3 只绑定 transport adapter；b4 拥有四段 relay、v2 切换、迁移桥和浏览器回归 |
