...
必须先用一批只针对语义的测试把它固定，再让四个插件依赖它。反过来做的话，插件迁移过程中
...
- **全局作用域注册表**：本故事不引入任何进程级单例——不加 `WeakMap<LifecycleScope>`、不加全局 `Set`、
  ...
  不需要任何跨实例登记（拆分理由见 D4）
- **DevTools 面板与作用域可视化**：`getEntries()` 只负责把数据取出来，怎么画不在本故事
...
> **`4b` / `5b` / `6b` / `9b` / `12b` 用后缀编号而不占新号**，是为了不打断已经写进 US-014 与 spec 的下游引用。
> AC#4b 固定首次 `dispose()` 失败后的重复调用（口径见技术笔记 D3「幂等的准确含义」——AC#4 的
> 「不抛错」在首次 reject 的场景下需要这条才不与 AC#7 / AC#8 冲突）；AC#5b 把 AC#5 的拒绝规则
> 扩到 `child()`（见下方脚注）；AC#6b 把 AC#6 的「disposer 内部回调本作用域」从 `acquire()`
> 扩到 `dispose()`——两者的答案不同：`acquire()` 抛错，`dispose()` 是空操作
> （详见下方「AC#6b 为什么不能靠 `state` 判定」）；AC#9b 给 `getEntries()` 立约，
> 让树形能在**释放之前**被断言而不是靠释放顺序反推（D4）；AC#12b 固定「分次 `acquire()` 时
...
> 正是本原语要消灭的失败形状。
>
> **AC#6b 为什么不能靠 `state` 判定**：`return () => scope.dispose()` 是插件里的主流写法
> （获取与释放写在一处，释放动作恰好就是「把这个作用域收掉」）。它跑到时 `state` 已是
> `disposing`，但外部的并发 `dispose()` 看到的也是 `disposing`——两者落在完全相同的内部状态上，
> 只有「调用发生在不在 disposer 的同步帧内」能把它们分开。判定圈大到整个释放过程就会
> 把外部并发调用误判成重入，AC#4 的「返回同一个 Promise 实例」随之失效。
>
> 反过来，disposer 在自己的 `await` **之后**再调本作用域的 `dispose()` 不在本条覆盖内：
> 那时已经出了同步帧，拿到的是本轮 in-flight 的 Promise，而它正等着这条 disposer 返回。
> 这种写法不支持，由 TSDoc 明令禁止——要在异步收尾里释放，释放的应该是**子作用域**。
>
...
**幂等的准确含义（AC#4 / AC#4b）**：`dispose()` 把**首次调用产生的那个 Promise 实例
...
未传 `label` 的条目填 `'anonymous'`（与 cordis 同名同义）。怎么把这棵树画出来不在本故事。

#### D5 — 一次 `acquire()` 只包一步会失败的获取
...
AC#12 与 AC#12b 合起来就是这条契约的可执行形式：前者固定「失败的那一步不留痕」，
后者固定「先前成功的那些步照常回滚」。迁移侧的对应写法见
[US-014 D7](US-014-plugin-scope-contract.md)。

### 已推迟：`acquireAsync()` 与它的取消出口
...
**它不在本故事范围内**：AC 表不含 `acquireAsync` 竞态用例，本故事只交付同步 `acquire()`。

#### 重新加回来的触发条件

出现下列任一情况即补上，并同时补齐取消语义的两个决策与三条竞态 AC（**不要**只加方法不加竞态测试）：
...
- 目前全链条**零**这样的调用方——为假想调用方冻结的竞态语义，等真实调用方出现时往往不合用
