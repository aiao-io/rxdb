...
| 6   | 一批 mutations 混入 QueryCache 实体与 Full/Filter 实体                                                                 | `EntityManager.mutations`                                      | 入口预检即拒绝，错误码固定复用 `mixed_versioned_cache_transaction`（[US-306 FR-046](../collaboration/US-306-working-tree-commits.md) 已定义，**不得另起名字**）；不得一部分写 changelog、一部分走 remote-then-local | ⬜   |
...
- [US-306 FR-046](../collaboration/US-306-working-tree-commits.md) — 兼容 cache 排除在 working tree 外，不实现 epic-006
