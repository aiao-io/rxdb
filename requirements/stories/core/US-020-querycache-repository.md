...
| 6   | 一批 mutations 混入 QueryCache 实体与 Full/Filter 实体           | `EntityManager.mutations`                                                                    | 拒绝，可判别错误；不得一部分写 changelog、一部分走 remote-then-local                                                                          | ⬜   |
...
| [packages/rxdb/src/__tests__/repository/](../../../packages/rxdb/src/__tests__/repository/) | A+B  | 生产路径 + 缓存质量；保留直接 `new` 的单元测试作为类级回归           |
