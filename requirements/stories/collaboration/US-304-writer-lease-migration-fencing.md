...
**未闭合项不再主动推进**，留到 1.0.0 发布前完善。涉及 AC6（挂起 writer 被迁移 epoch fence 的真实用例）
和已转给 [US-305](./US-305-commit-graph-head.md) 的 AC11（bridge ancestry、`oldBundlePolicy`、真实发布门禁）。

理由是**迁移路径至今没有投入使用**：0.0.x 线的 `RXDB_SYSTEM_SCHEMA_VERSION`(3)、
`RXDB_CHANGE_CODEC_VERSION`(1) 都未抬升，没有可迁移内容，清单切 `kind=migration` 必被门禁拒绝；
`v0.0.25` 桥接 tag 又因 squash 合并脱离发布主线，现在推进要先付一次「为发布而发布」的 bridge 成本。

**暂缓的是收尾工作，不是已交付的协议。** 下列内容仍在生产路径上，按正常代码维护，回归失败照修：

- `packages/rxdb/src/system/writer-lease.ts` 的状态机与错误码
- SQLite / PGlite 两个适配器在写事务内的 guard 校验与 lease 续租
- 四后端 `rowsAffected` conformance 套件、多进程 lease 注册套件

`requirements/migration-release.json` 保持 `kind=normal`——其中
`release.version === packages/rxdb/package.json.version` 那条校验是常态生效的，
用于发现 0.0.24 那类版本漂移，与本次暂缓无关，不要一并关掉。

> 恢复推进的触发条件：排 1.0.0，或出现任何需要抬升 `RXDB_SYSTEM_SCHEMA_VERSION` /
> `RXDB_CHANGE_CODEC_VERSION` 的需求。届时先补 AC6，再按下方「迁移发布的关闭条件」逐条走。
