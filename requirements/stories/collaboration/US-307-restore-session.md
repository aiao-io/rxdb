...
- **FR-026b**（已改口径，见 [epic-006](../../epics/epic-006-working-tree-commits.md)）：`bench-working-tree` MUST 包含「打开已有历史并恢复最近 commit」场景，报告 p50/p95，门禁用相对基线回归；固定 fixture 为 10,000 条实体记录 / 100 个 commit，基准环境为 Node + PGlite memory。不承诺浏览器 OPFS / IDB 下的同一数字。
...
- 拒绝路径（dirty、不可达 commit、跨库 commit）各有独立用例，断言工作树零变化。
