...
- [x] Testable: 「restore → 刷新 → 仍显示且标记未提交 → stage → commit」可独立验收
-->
...
**独立测试**：创建至少三个 commit，选择中间版本恢复，刷新后确认恢复结果，显式 stage 后再 commit，验证历史没有被覆盖。
...
- **FR-034**：restore / discard MUST 在同一数据库事务内校验 active branch token 与 expected head、working tree、index revision。初次 restore 要求 index 为空，成功只递增 working-tree revision；index 内容与 revision 保持不变。初次 restore CAS 失败时全部回滚且不创建 session；已有 session 的 commit/discard CAS 失败时保留工作树和 session，并由 expected/actual revision 派生 conflicted，不得自动选择任一 writer 的状态。discard 仅在用户后来 stage 过恢复结果时清空 index 并递增 index revision。
...
- restore 以当前分支父链上的兼容目标 commit 为数据源、以当前 HEAD 为工作树基线，产生普通、未暂存的 INSERT / UPDATE / DELETE 工作树变更；目标 commit 本身**不**被标记为「当前」。实现可以选择 baseline→target 正向重放或 HEAD→target 逆向重放，但必须先冻结路径并校验路径上的每个 ChangeSet。
