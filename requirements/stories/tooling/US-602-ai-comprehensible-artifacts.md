...
- [x] Valuable: 47 包选型是消费端 AI 今天答错的具体问题，症状见「病灶」
...
**我想要** 助手能说清「47 个 `@aiao/*` 包里我该装哪几个、按什么顺序组合」
...
2. **依赖声明自相矛盾，AI 会照抄错的那半边**。同一种「需要核心包」的关系有两种写法：
   ...
   核心被装成两份实例是已知故障模式。**复验方式**：`node -e` 读两个包的 `package.json` 对比。

3. **致命时序只写在 README 正文**。`rxdb.use()` 必须在 `connect()` 之前
   （[rxdb-plugin-tree README](../../../packages/rxdb-plugin-tree/README.md) 的「必须在 init() 之前 use()：
   仓储注册发生在 init() 内部」），以及事务内 `await entity.save()` 会落回队列并永久挂起
   （[rxdb README](../../../packages/rxdb/README.md)）。两条都不在 `.d.ts` 里，
   AI 只有主动 open README 才读得到。
...
- **A 阶段**：新增包关系真相源，至少覆盖四类事实——分层（core / adapter / plugin / binding / tool）、
  适配器互斥组、框架绑定与框架版本的对齐关系、组合时序约束（`use()` → `connect()`）
...
- 为 47 个包各写一份 Skill 或 `AGENTS.md`——维护成本乘以 47 且必然漂移，与唯一真相源冲突
- 把 `skills/` 塞进主包以外的任何包
...
| 4   | 病灶 2 的两种写法并存 | 跑门禁                                                 | 当前 `rxdb-adapter-wa-sqlite` 等包被拦下；改为 `peerDependencies` 后通过                   | ⬜   |
...
倾向 a：互斥组与分层本就是跨包事实，强行分散到 47 处反而制造 47 个漂移点。`api-surface.mjs`
...
`llms-full.txt` 覆盖 47 包全文，需在 B 阶段确认体积上界并决定是否按目录裁剪。
