...
|   P3   | HTTP 适配器 wire 级集成测试        | [US-213](stories/adapter/US-213-http-wire-integration-test.md)         | `http-protocol.md` 已随 US-212 的 `stable` 对外，后端照它实现却没有任何可执行验收；本包 7 个 spec 全在 `vi.stubGlobal('fetch')` 层拦截，transport 从未被真实 socket 打过 | 零依赖 `node:http` 参考后端（7 端点 + `received` 记录 + 6 个故障开关）+ 本地内存 fixture + `wire-integration.spec.ts`（17 条 AC）；`tests/` 需补进 tsconfig / eslint / coverage 三处纳管；**不改 `src/`** |
