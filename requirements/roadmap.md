...
仓库还剩 **10 条**未关闭故事（2 In Progress + 1 In Review + 7 Backlog，
...
> 线 E 的 AC#38/#39/#42 → [US-907](stories/future/US-907-devtools-manual-browser-regression.md)。

### 批次 2：已开工的线

> US-904 已 `Done`（阶段 A～D 全部关闭），已移出本表；其中只能人工在真实 Chrome 上做的
> AC#34/#38/#39/#42 拆成 [US-907](stories/future/US-907-devtools-manual-browser-regression.md)（Backlog，Low）。
...
> 已 Done（2026-09-01，AC#10 随三 OS 矩阵跑绿关闭，存证记在该故事里），三条均已移出本表。
...
| [US-906](stories/future/US-906-electron-devtools-developer-path.md)            | 代码侧已交付，6 条 AC 中 5 条 ✅。**只剩 AC#2 的人工半边**：按 README 跑一次 `nx dev dev-rxdb-electron` 并打开面板看一眼——**这一条只能由人来做，做完即可转 `Done`**，不占开发排期 |
