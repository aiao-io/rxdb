...
| 故事                                                                            | 为什么不排进批次 1                                                                                                                                                                                                                                                           |
| ------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [US-208](stories/adapter/US-208-electron-pglite-data-directory.md) **实现本体** | 选型已由批次 1 线 G 冻结为「IPC 事务 ID 协议」（2026-08-30），本行只剩实现，**已解除不开工限制**（约束 3）。开工时须带上线 G 冻结出的三条实现约束（`releaseOwner` 按 `webContents.id` 清场是前提、`begin` 必须超时 fail-fast 不得排队、AC#7 的主进程单实例须配 worker 隔离） |
