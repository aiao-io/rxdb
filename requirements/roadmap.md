...
三 OS 矩阵跑绿收掉 US-505 / US-208 后，仓库还剩 **9 条**未关闭故事（3 In Progress + 1 In Review + 5 Backlog，
...
| 顺序                                                                     | 解锁自     | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------------------------------------ | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| US-904 阶段 D                                                            | 线 E       | C2 已交付，阶段 D 已开工并基本落地：AC#48（snapshot wire 复用 `files.list`）、AC#50（preload 校验层）、AC#53（Electron conformance driver）已关闭；AC#52（真实全链路 E2E：`devtools-restart-persistence.spec.ts`）已于 2026-09-03 真机跑通，「本机不可验证」的判断作废。**AC#45 已于 2026-09-04 关闭**（同批修掉「授权档位送不进页内 connector」这处不成立的接线），**AC#46 / #49 亦已于同日关闭**（事件全集经 wire 逐类核对 + branch 双向核对；wire 侧强制调用的两条拒绝码）。**AC#51 亦已于同日关闭**——「只关闭不刷新」那条修掉了三处互相独立的缺陷（中继不通知断开、`#route` 未跳过协商帧、协商机 sessionId 构造时铸死），三者任缺其一都表现为「面板静默退回 v1 车道而守卫仍显示已连接」。**阶段 D 只剩 AC#47 一条 ⚠️**：面板侧下载从未接线——`v2-file-channel.ts` 的 `download()` 从不调用带 sink 的 `DevToolsPanelEndpoint.download()`，字节根本没到面板；接线涉及一个 UX 决定（WebKit 上没有 `showSaveFilePicker`），待 owner 判（注意：自定义 `app:` scheme 拿不到扩展 host permission，E2E 必须走应用的 `--serve` http renderer）。C 阶段残留 AC#34/#38/#39/#42 待人工浏览器回归 / 跨版本实证 |
| [US-905](stories/future/US-905-tauri-native-devtools.md) 阶段 1 → 阶段 2 | 线 E、线 C | **2026-09-04：harness 已建起来，AC#1 的 dev 半边关闭。** 形态不是 WebDriver（`tauri-driver` 在 macOS 上不存在）而是扩自检报告：`tauri-package-dev`（裸 `cargo build` 才有 `cfg(dev)`，`tauri build` 会打开 `custom-protocol` 把调试窗口整个编掉）+ 报告 schema v3（`windowLabels` 由 Rust 枚举、`devtools` 探针由主 WebView 上报）+ `devtools-smoke` target。实测 dev 产物窗口集合恰为 `["main", "rxdb-devtools"]`。**顺带修掉两处「定向投递从未成立」**：Rust 的 `Emitter::emit` 实为广播（要 `emit_to`）、JS 全局 `listen` 的 `Any` target 无视投递过滤（要 `getCurrentWebviewWindow().listen`）。**第三处已修**：`build-devtools` 的产物被 `build` 的 nx 缓存恢复抹掉（它跑在 `build` 之前、又没声明 `outputs`），现依赖反转 + 声明 `outputs`/`cache`，实测清空 `dist/` 后 20/20 全缓存命中而 `devtools/` 仍在位。**AC#1 因此关闭**（两个真实 WebView 完成 v2 握手）。**AC#2 仍 ⚠️**：80 条 conformance 今天跑在进程内 relay 上，不是这两个真实窗口之间——握手打通不等于它。AC#3～#6 未动，阶段 2 未开工                                                                                           |
...
| ~~US-906~~                                                                     | **已交付（2026-09-03）**：`build-desktop-dev` 变体 + README 桌面端调试一节 + 面板 `unsupported` 给原因 + e2e 收敛到同一份产物。6 条 AC 中 5 条 ✅，AC#2 只差人工那一次 dev 流程 |
