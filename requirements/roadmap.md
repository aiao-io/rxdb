...
| [US-217 本地数据库一致性备份与恢复](stories/adapter/US-217-local-database-backup-restore.md) | 🚧 In Progress | 阶段 A（PGlite）已交付，公共契约冻结在 `@aiao/rxdb` 核心，AC#9 峰值内存只有结构性证据、AC#14 未在启用 storage 插件的库上实跑；阶段 B（SQLite 共享层）复用 A 的契约；阶段 C 阻塞于三 OS packaged smoke | 批次 3   |
