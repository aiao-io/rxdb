...
| US-212 AC#29 SSE / invalidation                         | **已移出 US-212**（2026-08-24 owner 判定）。需要一个 core 侧**今天不存在**的抽象：远端适配器可调用的失效上报口。现存的失效状态只有 [`QueryCacheSyncMemo`](../packages/rxdb/src/repository/query-cache-sync-memo.ts)（US-020 D13），三条失效路径全由 core 内部触发、`clear()` 无对外出口；[`QueryManager`](../packages/rxdb/src/repository/QueryManager.ts) 的重跑只认 `ENTITY_LOCAL_*` 三个本地事件。`RxDB.dispatchEvent()` 虽是公开方法，但用本地写事件冒充远端变更**既是语义撒谎又清不掉 memo**——被触发的重跑会在记忆窗口内跳过同步。今天缺它不产生错误结果，只是没有实时性（下一次 `find()` 照常回远端校验），按「价值待证」不建故事文件。**解锁条件 = 出现一个真实的实时性需求，且能说清失效粒度（整实体 / `where` 指纹 / id 集合）** |
