...
| [US-905](stories/future/US-905-tauri-native-devtools.md) 阶段 1 → 阶段 2 | 线 E、线 C | **2026-09-04：harness 已建起来，AC#1 的 dev 半边关闭。** 形态不是 WebDriver（`tauri-driver` 在 macOS 上不存在）而是扩自检报告：`tauri-package-dev`（裸 `cargo build` 才有 `cfg(dev)`，`tauri build` 会打开 `custom-protocol` 把调试窗口整个编掉）+ 报告 schema v3（`windowLabels` 由 Rust 枚举、`devtools` 探针由主 WebView 上报）+ `devtools-smoke` target。实测 dev 产物窗口集合恰为 `["main", "rxdb-devtools"]`。**顺带修掉两处「定向投递从未成立」**：Rust 的 `Emitter::emit` 实为广播（要 `emit_to`）、JS 全局 `listen` 的 `Any` target 无视投递过滤（要 `getCurrentWebviewWindow().listen`）。**第三处已修**：`build-devtools` 的产物被 `build` 的 nx 缓存恢复抹掉（它跑在 `build` 之前、又没声明 `outputs`），现依赖反转 + 声明 `outputs`/`cache`，实测清空 `dist/` 后 20/20 全缓存命中而 `devtools/` 仍在位。**AC#1 / AC#4 因此关闭**（两个真实 WebView 完成 v2 握手；同 label 重开拿到另一个 session）。AC#4 顺带补上 Tauri 侧缺失的「对端没了」信号——与 Electron 侧 US-904 AC#51 同形，不修则 connector 的 session 永不释放。**AC#3 亦已关闭**（真窗口伪造身份被 Rust 白名单拒掉）。**AC#2/#5/#6 仍 ⚠️**，三条都不是纯写用例：AC#2 要逐例现装配会话，真实窗口的 connector 是固定档位单例，得在 demo 里再建一套 conformance bootstrap（往产品塞测试脚手架的边界需 owner 定）；AC#5 卡在一处镜像缺陷——主窗口刷新后**面板侧**不重新协商（与已修的 connector 侧对称），已标 `it.fails` 待 owner 判；AC#6 需阶段 2。另修掉「调试窗口从来没有样式」。阶段 2 未开工 |
