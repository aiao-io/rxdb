...
| [US-905](stories/future/US-905-tauri-native-devtools.md) 阶段 1 → 阶段 2 | 线 E、线 C | **2026-08-31 的 2 ✅ 已推到 5 ✅ / 3 ⚠️（#2 #6 #7 未关闭）。** harness 已建起来（`tauri-package-dev` + 自检报告 schema v5 + `devtools-smoke`，不走 WebDriver）。AC#1/#3/#4/#5 均以**两个真实 WebView**为证据关闭。同批修掉五处缺陷，其中影响面最大的是**调试窗口从来没有样式**（vite 配置缺 tailwind 插件、也没有样式表入口，整个面板版面全错而 e2e 照常绿）。**AC#7 由 ✅ 改回 ⚠️**——复核发现那 80 条 conformance 的「Tauri driver」是个透明转发节点，链路里没有一次 `invoke`。**AC#2 只剩「五类操作在真实双窗口间跑一遍」**（跨窗口投递与关闭/重开已覆盖）；把 80 条断言整套搬到真实窗口复跑**明确不做**，理由见故事。AC#6 需阶段 2 或一个 dev-only 后端开关。阶段 2 未开工 |
