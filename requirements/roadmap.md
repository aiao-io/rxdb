...
|   P2   | 提交图与 HEAD 持久化       | [US-305](stories/collaboration/US-305-commit-graph-head.md)       | **已不再是「下一步」**：spec 重生成已完成，US-305 连同 epic-006 另外三条故事的代码已在 `next-0912` 上落地并转 👀 In Review（2026-09-18）。本行保留是因为**排期口径没变**——桥接发布（线 A）仍只卡**迁移发布**，`bridge.tag` 依旧是 `null`，整链（完成计划批次 4）仍排在所有其他批次之后 | 独立命名空间的新契约、commit 存储布局、baseline commit 与一次性迁移                                                                                      |
