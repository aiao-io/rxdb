...
|   P3   | HTTP 适配器 wire 级集成测试        | [US-213](stories/adapter/US-213-http-wire-integration-test.md)         | `http-protocol.md` 已随 US-212 的 `stable` 对外，后端照它实现却没有任何可执行验收；本包 9 个 spec 里 6 个在 `vi.stubGlobal('fetch')` 层拦截（另三个零桩纯单元测试），transport 从未被真实 socket 打过 | 零依赖 `node:http` 参考后端（7 端点 + `received` 记录 + 7 个故障开关）+ 本地内存 fixture + `wire-integration.spec.ts`（17 条 AC）；`tests/` 需补进 tsconfig / typecheck target / eslint / coverage 四处纳管；**不改 `src/`** |
...
| [US-213](stories/adapter/US-213-http-wire-integration-test.md)                  | 零前置（US-212 已 Done），但**不解锁任何人、也不是决策债**，两条进批次 1 的理由都不成立。它防的是存量风险：`http-protocol.md` 已随 `stable` 对外，后端照文档实现却无可执行验收——风险随第三方后端接入数增长而非随时间增长，因此可以排后但不宜无限推。纯测试资产，不改 `src/`；若参考后端暴露协议本身不自洽，按约束 13 另开故事，不在本行内改协议                                                                                                                                                                                                                                                                                                                                                                          |
...
| 项                                                      | 判定                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| US-015 阶段 B（插件间依赖图）                           | **已移出 epic-008 承诺范围**（2026-08-21）。全仓库唯一的 `inject` 是 search 的 `['adapter:local']`，零 `plugin:*` 消费方——拓扑序与环检测是为一个不存在的依赖图准备的。**解锁条件 = 出现第一个 `plugin:*` 依赖声明**（约束 8，这是判据不是建议）。**连带结论（2026-08-22 核对）：US-015 的 `In Review` 是稳态，不是过渡态。** 其余 11 条未关闭故事（US-305～308 / US-904 / US-905 / US-208 / US-211 / US-213 / US-703 / US-505）**没有任何一条会产生 `plugin:*` 消费方**，所以状态汇总里那个「👀 1」在可预见的排期内是常量。读表时不要把它当待办。（这个数字在 2026-08-27 内变动过两次：US-215 随 US-214 关闭补进来使其成为 12，同日 US-021 / US-022 / US-215 三条全部关闭后落回 11；结论从未变过，变的只是计数）                          |
