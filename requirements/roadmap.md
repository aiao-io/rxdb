...
| :----: | -------------------------- | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
|   P1   | 实例级实体同步配置覆盖     | [US-026](stories/core/US-026-instance-sync-override.md)           | HTTP demo 的前后端仍以两个实体类表达不同同步策略，存在可复验的重复                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 初始化配置整体覆盖、实例隔离、三框架契约与 demo 单类收敛                                                                                                 |
...
|   P2   | 本地数据库一致性备份与恢复 | [US-217](stories/adapter/US-217-local-database-backup-restore.md) | US-207 / US-208 / US-210 都把导入导出与热备份排除在范围外，今天唯一路径是退出应用后整目录复制（[US-504](stories/plugin/US-504-electron-local-file-storage.md) / [US-505](stories/plugin/US-505-tauri-local-file-storage.md) AC#3），不是可由应用调用的一致性备份接口                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 事务边界上的带完整性校验快照、恢复到空目标的独占语义、阶段 A（PGlite）→ B（SQLite 共享层）→ C（桌面 host）；外置文件本体、跨引擎迁移与密钥轮换均不在范围 |
|   P2   | 提交图与 HEAD 持久化       | [US-305](stories/collaboration/US-305-commit-graph-head.md)       | **已不再是「下一步」**：spec 重生成已完成，US-305 连同 epic-006 另外三条故事的代码已在 `next-0912` 上落地并转 👀 In Review（2026-09-18）。收尾回归同日跑完，`tasks.md` **133 条已全部关闭**，6 后端 × 2 套件 5031 条零失败；T109 / T132 于同日随 `bench-working-tree` 的参考基线重新冻结而闭合（走契约 §3.1 允许的「测点集合变化后重新冻结」，不是失败后重算），门禁四项 `✓ PASS`。**但那份基线是带机器负载的初版**：`frozenAbsolute.commit` 虚高 29%，机器静默复冻之前不得作为发布放行依据；`status` 的容差口径仍归评审（见 [status-overview](status-overview.md#本地工作树与提交历史) 理由 ①）。本行保留是因为**排期口径没变**——桥接发布（线 A）仍只卡**迁移发布**，`bridge.tag` 依旧是 `null`，整链（完成计划批次 4）仍排在所有其他批次之后 | 独立命名空间的新契约、commit 存储布局、baseline commit 与一次性迁移                                                                                      |
...
> 线 E 的 AC#38/#39/#42 → 已移出承诺范围（人工浏览器回归，项目早期暂不做，未来收尾另立故事）。

### 批次 2：已开工的线

> US-904 已 `Done`（阶段 A～D 全部关闭），已移出本表；其中只能人工在真实 Chrome 上做的
> AC#34/#38/#39/#42 已移出承诺范围（项目早期暂不做人工回归，未来收尾另立故事）。
> [US-905](stories/future/US-905-tauri-native-devtools.md) 已 `Done`（2026-09-14，阶段 1 八条 +
> 阶段 2 九条共十七条 AC 全部关闭，三平台 devtools smoke 全绿，win32 首跑里 idb 档的
> SharedWorker 挂起修于 dedicated Worker 传输，存证见该故事），已移出本表。
...
| 故事                                                                             | 为什么不排进批次 1 / 当前状态                                                                                                                                                                                                                                                                                      |
| -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [US-026](stories/core/US-026-instance-sync-override.md) 实例级实体同步配置覆盖   | 基于现有策略解析，无桥接发布前置；与 US-025 协调消费者接缝，不等待其阶段 C～E。关闭以实例隔离、三框架契约和前后端单类 demo 全部通过为准                                                                                                                                                                            |
...
| [US-217](stories/adapter/US-217-local-database-backup-restore.md) 阶段 A → B → C | 不依赖工作树 / commit graph，也无桥接发布前置，故与 US-026 / US-211 同批。阶段 A 须先验证 PGlite 有界内存与目标存储的原子恢复能力（故事 INVEST 卡的 Estimable / Small 两项尚未勾选），按价值排在 US-026 之后。一个 PR 只交付一个阶段；尚未交付的组合必须明确拒绝备份与恢复，**拒绝行为通过测试不等于该组合已支持** |

### 批次 4：epic-006 链（整体压后）

> **有意排在最后**：这条链今天不是优先交付项——先把批次 1～3 的价值交付完，再推进本批次。
>
> **线 A 不是开工闸门**：它由 owner 手动发起、手动决定时点，挡的是 epic-006 的**迁移发布**
> （`bridge.tag` 为 `null` 时 `kind=migration` 门禁必红），不挡 US-305 / US-306 / US-307 / US-308 的
> 编码、测试与合入。唯一的真开工前置是 `specs/001-working-tree-commits/` 的重生成（见
> [epic-006 依赖顺序](epics/epic-006-working-tree-commits.md#依赖顺序) 第 2 步）。
> 线 A 启动时，[release-plan.md](release-plan.md) 的执行顺序与两条硬前提全部照旧生效
> （动手前按[零散收尾项](#零散收尾项不成故事随手可带)那唯一一条重跑 `dry-run` 实测区间）。

| 线 / 链             | 内容                                                                                                                                                                                                                                                                             | 解锁自                                                                                                                                  | 关闭判据 / 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A｜桥接版本发布** | 按 [release-plan.md 的执行顺序](release-plan.md) 走完 0～6 步，发一个 `kind=bridge` 的**非迁移**版本                                                                                                                                                                             | —                                                                                                                                       | 五条**全部**成立才算完：① `release.version` **≠ `0.0.25`**（今天清单里的 `bridge/0.0.25` 是 0.0.25 那次发布的如实记录，不是本次成果，不许当判据用、不许改写）；② 与 `packages/rxdb/package.json` 同值；③ tag 已推送且 `git merge-base --is-ancestor v<版本>^{commit} HEAD` 人工跑过并留证；④ `migration-release-gate --release-tag=v<版本>` 全绿；⑤ 回写 [US-305](stories/collaboration/US-305-commit-graph-head.md) 的 FR-030 / AC14 证据。**④ 单独没有区分力**（四条 bridge 钩子只对 `kind=migration` 生效，桥接发布走不到它们，今天不做任何事直接跑就是绿的）；真正有区分力的是 ① 和 ③，这两条在**发布当下没有任何自动化在守**。① 只有一条**延迟**防线：真发成 `0.0.25` 的话，要等下一次 migration 引用它时才会撞 `bridge.version` 下限而红。另有一条同样无自动化的隐含判据——桥接**不得抬升** `RXDB_SYSTEM_SCHEMA_VERSION` / `RXDB_CHANGE_CODEC_VERSION`，门禁只比对布尔位、从不读源码常量，须按 [release-plan 硬前提 1](release-plan.md) 的两条 `git log -G` 人工复测（`-S` 恒空、会给出假清白） |
| epic-006 链         | [US-305](stories/collaboration/US-305-commit-graph-head.md) → [US-306](stories/collaboration/US-306-working-tree-commits.md) 阶段 A → B → C →（[US-307](stories/collaboration/US-307-restore-session.md) ∥ [US-308](stories/collaboration/US-308-branch-isolation-conflict.md)） | specs 重生成（[epic-006 依赖顺序](epics/epic-006-working-tree-commits.md#依赖顺序) 第 2 步）；线 A 只解锁本链的**迁移发布**，不解锁开工 | epic-006 的固定顺序（约束 5），**不可交换**。US-307 / US-308 的核心持久层半边可与 US-306 阶段 C 并行开工，但三框架入口与 benchmark 采样必须复用阶段 C 冻结的 `useWorkingTree()` 与 `bench-working-tree`；每条故事按各自 AC 关闭                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
