...
> rxdb-model UI 派生以它为依托，见[立项池](#立项池待-owner-决策)。
...
> 批次 1（2026-08-29 / 08-30）与批次 2 已全部交付，正文不再保留。
...
> 代码已在 `next-0912` 上全部落地并转 👀 In Review（2026-09-18）：`specs/001-working-tree-commits/tasks.md`
...
**线 A 启动前必读**（2026-09-12 实测的 dry-run 三条坑与非规范提交问题，动手前必须复测）：
...
1. **线 A 启动前先跑 `nx release version --dry-run` 看真实版本号**（线 A 已压后到批次 4，启动时执行）。
   ...
   - **默认推算出来的就是 `0.0.25`**——正好是线 A 关闭判据 ① 的禁用值，且 npm 上已被占用。
     原先预计「会算成 minor bump `0.1.0`」是**错的**：specifier 解析成 `minor`，但 nx 的
...
| 项                                                                          | 判定                                                                                                                                                       |
| --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| US-015 阶段 B（插件间依赖图）——历史行                                       | 曾在此表：判词「为一个不存在的依赖图准备」，解锁条件 = 出现第一个 `plugin:*` 依赖声明。US-025 阶段 C/D 成为消费方后，该行随 US-015 整体 `Done` 移出        |
