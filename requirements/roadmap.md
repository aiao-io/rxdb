...
|   P2   | 提交图与 HEAD 持久化       | [US-305](stories/collaboration/US-305-commit-graph-head.md)       | 旧暂存导出已在 `0.0.24` 删除，能力缺口现在完全敞开；开工卡的是 `specs/001-working-tree-commits/` 重生成，桥接发布（线 A）只卡**迁移发布**。**整链排期压后**（完成计划批次 4），先交付其余价值                                                                        | 独立命名空间的新契约、commit 存储布局、baseline commit 与一次性迁移                                                                                      |
...
仓库还剩 **8 条**未关闭故事（1 In Progress + 0 In Review + 7 Backlog，
