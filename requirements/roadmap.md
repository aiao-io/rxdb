...
| 🚧 P1  | HTTP 远程适配器                    | [US-212](stories/adapter/US-212-http-adapter.md)                       | **产品优先级明确要求靠前。** 已有 REST API 没有 RemoteBase 可挂，今天只能绑 supabase。必须是独立 `adapter:remote` + 独立注册 sqlite 行缓存，禁止 HTTP 内嵌 sqlite              | ~~阶段 A handlers + QueryCache ducks + 翻页/分块契约 + 错误分类 + **结构隔离不变量**（约束 11）~~ ✅ 2026-08-23 关；余阶段 B REST mapping。**发布门禁已全部解除**（约束 10） |
