...
> 线 E 的 C1（面板抽取）已于 2026-08-29 交付（`modules/rxdb-devtools-panel/`），剩 C2；
...
| **E｜US-904 的零前置半**      | [US-904](stories/future/US-904-devtools-native-storage-contract.md) 阶段 A 已 ✅（`decision: supported`）；C1（面板抽取，行为中性）已于 2026-08-29 交付，剩 C2 | 阶段 A 零前置，且是阶段 D 的门禁：判 `unsupported` 则阶段 D 整段不做。C1 是行为中性重构，阶段 B 已交付，不必等任何东西（C1 已交付，见 `modules/rxdb-devtools-panel/`）                                                                                                                                           | 阶段 A 与 C1 均已达成（C1：面板抽成私有 Angular library `modules/rxdb-devtools-panel/`，行为零变化，2026-08-29）；剩 C2。阶段 A 带出两条阶段 D 约束：Electron 43 缺整个 `chrome.permissions` 命名空间（需显式能力探测，禁静默 fallback）、扩展面板只在 dock 模式 DevTools 中注册                                                                                                                                                                                                                                                                               |
