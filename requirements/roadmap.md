...
> 线 E 的 C1（面板抽取）已于 2026-08-29 交付（`modules/rxdb-devtools-panel/`），C2 已于 2026-08-30 交付
> （面板改接 v2、私有 OPFS 状态机删除、扩展 e2e 立项；AC#38/#39 待跨版本实证，#42 随 #34 的人工回归）；
> 线 G 已于 2026-08-30 交付（两案对照实验跑完，选型冻结为「IPC 事务 ID 协议」）。
...
| **E｜US-904 的零前置半** ✅   | [US-904](stories/future/US-904-devtools-native-storage-contract.md) 阶段 A 已 ✅（`decision: supported`）；C1（面板抽取，行为中性）2026-08-29 交付，C2（四段 relay v2 切换）2026-08-30 交付 | 阶段 A 零前置，且是阶段 D 的门禁：判 `unsupported` 则阶段 D 整段不做。C1 是行为中性重构，阶段 B 已交付，不必等任何东西（C1 已交付，见 `modules/rxdb-devtools-panel/`）                                                                                                                                           | 阶段 A、C1、C2 均已达成（C2：面板 v2 数据面、浏览器 provider、导出停用、`apps/rxdb-devtools-extension-e2e` 真实四段中继，2026-08-30；e2e 当场抓到「v2 帧被私有端口吞掉、真实 Chrome 稳定退回 v1」并已修）。AC#38/#39 需跨版本产物与真实断连实证、#42 随 AC#34 的人工浏览器回归，均不在代码工作范围。阶段 A 带出两条阶段 D 约束：Electron 43 缺整个 `chrome.permissions` 命名空间（需显式能力探测，禁静默 fallback）、扩展面板只在 dock 模式 DevTools 中注册                                                                                                    |
| **G｜US-208 两案对照实验** ✅ | 让「IPC 事务 ID 协议」与「adapter 完整托管主进程」两案各过同一套事务与事件测试，产出选型结论                                                                                                | 从批次 3 拆出来的**决策债**，不是开发债：零前置，且结论会显著改变 US-208 的规模估计。与线 E 阶段 A 同性质——先花小钱做实证，避免大钱花错方向。捆在实现里等于把决策也一起推迟                                                                                                                                      | 已达成（2026-08-30）：两份可运行原型跑在真实 Electron 44 主进程 + 真实 `ipcRenderer.invoke` 上（`apps/dev-rxdb-electron/tools/pglite-tx-experiment/`），事务体两案同源，对照断言在 `apps/dev-rxdb-electron-e2e/src/pglite-tx-experiment.spec.ts`。结论：语义四项（事务内读写、中途抛错、类型保真、事件只送已提交）**完全打平**，选型由接口面斜率与崩溃语义决定，**冻结为「IPC 事务 ID 协议」**（约束 3 已履行）。顺带量到一条两案共有的新约束：PGlite 的 WASM 同步跑在主进程 JS 线程上，一条 2 秒查询让主进程停摆 2007ms，影响 US-208 AC#7。实现本体留在批次 3 |
...
| 故事                                                                                             | 为什么不排进批次 1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [US-208](stories/adapter/US-208-electron-pglite-data-directory.md) **实现本体** ✅（2026-08-30） | 已按批次 1 线 G 冻结的「IPC 事务 ID 协议」实现，故事置 `In Review`：**AC#1～#9、#11 关闭**，证据为 `pglite-data-directory.spec.ts`（真实临时目录重启，JSONB / 数组 / bigint / binary 逐值保真）、`pglite-transaction-contract.spec.ts`（复用共享事务套件 11 条，零跳过项）、`pglite-open-failure.spec.ts`（open 失败可判别、不留空库、不回退 memory）、`electron-pglite-host.spec.ts`（握手 / 会话清场 / 权限）。线 G 的三条约束全部落地。顺带修掉一个产品缺陷：`transaction_pglite_result` 未解码 `rxdb_change` 行，浏览器档位被一次 NOTIFY 重读掩盖，桌面档位暴露。**AC#10（三平台打包 smoke）仍 ⬜**：打包接线已写完但本机无法验证，需一次真实三 OS `release-desktop.yml` `workflow_dispatch`，本轮按边界**未触发** |
...
3. US-208 与 US-210 均排在 US-207 之后，复用其抽出的 host 契约。US-210 的事务方案已冻结：
   ...
   且崩溃后事务仍照跑照提交（无取消点）被否决。本条的前置动作已履行，US-208 实现本体可开工。
