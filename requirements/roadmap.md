...
> US-208 实现本体已交付并转 `In Review`（只欠 AC#10 的三平台打包 smoke，已排在下方完成计划批次 3），不再作为「建议」列出。US-306 / US-307 / US-308 不在本表单列——它们是 US-305 的后续交付，排期跟随
...
| [US-905](stories/future/US-905-tauri-native-devtools.md) 阶段 1 → 阶段 2 | 线 E、线 C | 阶段 1 门禁已解除，AC#1～#8 已全部落地并通过本地验证（两个硬阻塞：`rxdb-devtools` 窗口 capability、`devtools_message` 的 `#[cfg(dev)]` 隔离均已破；Rust 25 + TS 217 + conformance 80 断言全绿）。**按 AC 判据逐条核过后：AC#7/#8 ✅，其余 6 条 ⚠️**——判据里都含「真实 Tauri 窗口 / 真实构建产物」那一半。**卡的不是构建环境**：本机 tauri-cli 2.11.4 齐全，`nx run dev-rxdb-tauri-e2e:desktop-smoke` 会真打出 release 二进制再驱动它（2026-09-01 实测 13 tests 全绿）；卡的是**没有驱动两个真实 WebView 的 harness**——`desktop-smoke` 按 US-210 AC#9 刻意只做进程级驱动、不上 WebDriver，所以双 WebView 握手、AC#4 的「同 label 重开拒旧身份」、AC#5 的退出释放都还验不到。阶段 1 的代码侧收尾**已做完**（2026-09-01，明细见故事的「交付状态」，本表不再复述）。阶段 2（真实 US-210 SQLite / US-505 native files）等 US-505 关闭后按独立 PR 交付 |
...
> 全部照旧生效（动手前按零散收尾项第 2 条重跑 `dry-run` 实测区间）。
