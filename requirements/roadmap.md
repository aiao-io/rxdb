...
| :----: | ---------------------------------- | ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
|   P1   | QueryCache 接入统一 Repository     | [US-020](stories/core/US-020-querycache-repository.md)                 | **US-212 的唯一硬前置，因此升到全表最前。** 配了 `SyncType.QueryCache` 今天是空操作：find 打本地、save 进 changelog。类存在、supabase ducks 存在，生产路径从不实例化 | 阶段 A 接线（**解锁 US-212 发布**）；阶段 B orphan / 指纹 / SWR SQL / 错误分类（**解锁 US-212 标 stable**）。不 inherit US-203 AC#6      |
