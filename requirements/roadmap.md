...
### epic-006 评审顺延的架构项（2026-09-24 从 review 报告转入）

`next-0912` 评审报告收口时留下的六项，加上 2026-09-25 复核 `review` 分支（即这一轮收口本身）时
顺延的五项（表末五行）。**每一项的判据、修法与「不做的理由」都写在对应代码的 TSDoc 里**，
...
| `WorkingTreeEntry.fingerprint` 要不要从 FNV-1a/32 换成 SHA-256                    | 今天不挡——没有生产代码比较这一列；US-308 冲突检测按身份逐对比较之后才生效，误判相等约 2⁻³² / 对                                                                            | 由 US-308 定。代价是捕获热路径上每次写多算一遍纯 JS 摘要、列宽 8 → 64 位；同步的 `sha256Hex` 现成，不需要异步也不需要新依赖。判据见 `packages/rxdb-plugin-working-tree/src/working-tree/capture-runtime.ts` 的 `fingerprintOf` `@remarks`                                                                                                                    |
