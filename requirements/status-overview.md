...
1. 数字由 `grep -h "^status:" requirements/stories/*/US-*.md | sort | uniq -c` 推导，**请勿手写维护**；合计等于 `stories/*/US-*.md` 里**带 `status:` frontmatter 的**文件数（62 个文件 − 1 个 [US-904 阶段 A 可行性记录](stories/future/US-904-phase-a-evidence.md)，那是证据留档不是故事），epic 文件不计入。
2. 其中 **11 条是多阶段故事**（[US-012](stories/core/US-012-field-semantic-metadata.md)、[US-015](stories/core/US-015-plugin-inject-dependency.md)、[US-020](stories/core/US-020-querycache-repository.md)、[US-023](stories/core/US-023-querycache-remote-invalidation.md)、[US-207](stories/adapter/US-207-desktop-local-database.md)、[US-210](stories/adapter/US-210-tauri-sqlite-local-database.md)、[US-211](stories/adapter/US-211-multi-miniprogram-platforms.md)、[US-212](stories/adapter/US-212-http-adapter.md)、[US-214](stories/adapter/US-214-http-browser-demo.md)、[US-306](stories/collaboration/US-306-working-tree-commits.md)、[US-904](stories/future/US-904-devtools-native-storage-contract.md)）：一个编号一个文件一条状态，正文用「交付阶段」表分批交付，**全部阶段关闭后才置 `Done`**。阶段不单独计数，见 [README](README.md#大故事用交付阶段不用子故事文件)。
...
- ✅ [US-213 HTTP 适配器 wire 级集成测试](stories/adapter/US-213-http-wire-integration-test.md) — US-212 的验收补票，2026-08-27 关闭，**未重开该故事**：本包 9 个 spec 里 6 个在 `vi.stubGlobal('fetch')` 层拦截（共 10 处桩，另三个零桩纯单元测试），transport 从未被真实 socket 打过，`http-protocol.md` 已随 `stable` 对外却无可执行验收。零依赖 `node:http` 参考后端（只 import `node:*`，**不引用被测包的任何类型**，否则「参考实现」退化成「实现的镜像」，两边可以一起错还一起绿）+ 真实 fetch，17 条 AC 全绿：45 条 wire 用例、包内 350 条全绿，`tests/**` 里 `vi.stubGlobal('fetch')` 出现 0 次（仅三处注释解释为什么不打桩）、`it.fails` / `.skip` / `.todo` 0 处（约束 13 的 expected-fail 出口一次都没用上，即协议本身没被打出缺陷），`src/` 一行未动，覆盖率与关闭前逐位相同（`tests/**` 不进分母）。**两条 AC 在收口审计中被发现「绿得不作数」并当场补强**：AC#16 只比对两次结果相等，一个从不实现 304、每次都回 200 的后端照样全绿，补断服务端**写出**的状态码（`200` → `304`）；AC#6 的乱序后端只改了首页已发出的行，冻不冻结果都一样，改成「增 / 删 / 改各一行且都指向后面几页」并加三条 `server.read` 前置断言，再断行内容而非 id 列表——只冻 id 的实现会交出被改新的 `updatedAt`，而 core 拿它比新鲜度，结果是本地**跳过一次本该发生的拉取**。逐条见故事的[落地偏差](stories/adapter/US-213-http-wire-integration-test.md#落地偏差)
...
- 🅰️ ✅ [US-023 QueryCache 远端变更的失效上报口与实时同步](stories/core/US-023-querycache-remote-invalidation.md) — 三阶段单文件故事，2026-08-27 建档。**承接 [US-212 AC#29](stories/adapter/US-212-http-adapter.md)**：那条 2026-08-24 因「拿不到 owner」移出、登记进 [roadmap「明确不排期」](roadmap.md#明确不排期)，其解锁条件（真实实时性需求 + 说得清失效粒度）现已满足——US-214 的 demo 上两个窗口改同一份数据谁也看不见谁。断的是三层且每层都是结构性的：`RxDBAdapterRemoteBase` 的抽象成员清一色是客户端发起的拉取、`QueryCacheSyncMemo` 的三条失效路径全由 core 内部触发且 `clear()` 无对外出口、`QueryManager` 只监听三个 `ENTITY_LOCAL_*`。**失效粒度判定为整实体**（`where` 指纹要求后端有状态；id 集合省不掉那次 metadata 往返——「某 id 是否落进某个 `where`」本地判定不了），扩散沿既有 `depEntityTypeMap` 覆盖关系查询。**通知只带实体名不带行数据**（广播给所有订阅者，推行数据在多租户后端上即越权泄露），**不做轮询降级**（说不清延迟上界），**重连即全量失效**（断线期间的变更无人补发）。抽象数 1（core 失效上报口）：适配器侧照 supabase 先例在包内自持通道，不加契约成员，US-212 AC#19 的结构隔离不变。**三个阶段均于 2026-08-27 交付，31 条 AC 全绿，故事置 `Done`。** 阶段 A：core 失效上报口 + 事件 + devtools 清单（AC#1–#11 / #26–#31）。阶段 B：`@aiao/rxdb-adapter-http` 的 SSE 通道 `change-feed.ts`，缺省关闭（AC#12–#19，包内 391 条用例绿）；通道按 D4 **不加适配器契约成员**，只靠三个回调（`clientId` / 实体清单 / 失效上报）接触适配器，D5 的「不做轮询降级」落成 `onUnavailable` 诊断回调，运行时没有全局 `EventSource` 时**不重连**。阶段 C：参考后端广播（**协议端点与订阅者登记分文件**——前者要照抄，后者是 demo 设施）+ demo 的 `?changefeed=1` 开关与计数面板 + 双 browser context 的 e2e（AC#20–#24，16 条 e2e 全绿）。**两个页面必须是两个 context**：同 context 共享 OPFS / IndexedDB 与同一个 `BroadcastChannel`，测出来的会是 US-009 的跨 tab 同步而不是这条通道。开关缺省关是硬要求（D11）——demo 把 `syncStaleTime` 设成 `0`，通道一开，US-214 那些**数请求条数**的既有用例会集体变红，而它们守的是本来就正确的行为；关掉时的现象反过来被冻成对照用例（AC#23）。**阶段 B 的一句判断在阶段 C 被推翻**：原以为阶段 C 只动 `apps/`，实际 AC#24 的「抑制了几条回声」在包外结构上不可见（抑制发生在适配器内部，被抑制的通知不会上报失效，从 core 的事件流上看与「没收到」相同），因此补了与 `onUnavailable` 对称的 `onNotification` 出口，字段集固定五个、结构上带不了行数据，D8 不受影响
  - ⬜ 阶段 A core 失效上报口 — 清记忆与重跑活查询必须同一入口、**先清后跑**（反过来会命中记忆窗口、读回同一份陈旧本地行，即 US-212 判定点名的偷渡）；**不得单独关闭故事**，没有调用方的失效入口正是 US-015 阶段 B 被移出时的那句判词
  - ⬜ 阶段 B HTTP 适配器变更通知通道 — 可选 SSE、缺省关闭（形状对齐 `conditionalRequests`）、`clientId` 自回声抑制、退避重连、连不上时报诊断信号（US-215 先例）；协议文档补「变更通知（可选）」一节
  - ⬜ 阶段 C demo 与双页面 e2e — 后端广播 + `?changefeed=1` 开关（缺省关，否则打破 US-214 既有 e2e 的请求计数断言）+ 两个 browser context 的收敛用例与关掉开关的对照用例
- ✅ [US-022 QueryCache 远端行的列契约与缺列诊断](stories/core/US-022-querycache-remote-row-contract.md) — 出自 US-214，2026-08-27 关闭。`upsertMany` 的列清单取自 `Object.keys(data[0])`，是绕开仓储的裸 SQL 写，实体 `default` 不参与；而 `http-protocol.md` §2 自己的示例行就没有 `createdAt`，照抄它实现后端第一次拉取就撞 `NOT NULL constraint failed: public$recipes.createdAt`——本地表名 + 远端不存在的列名 + 适配器内部的栈，三重误导。新增 `assertQueryCacheRowContract`（落在 `rxdb-adapter-sqlite-core`：判据是「`create_table_sql` 会把哪些列建成 NOT NULL 且不给 DEFAULT」，逐条对齐 `create_table_column_sql`，两处必须同改），**判在 `this.transaction()` 之外**，因此「一批一行都没落地」不靠 ROLLBACK 兑现，用例直接断言客户端连 `BEGIN` 都没收到。顺带收口同一处代码的相邻风险：批内异构行今天是绑 `undefined` 写成 NULL、把可空列**静默清空**，现在一并拒批。契约正文归 `sync.md`（Supabase 远端同路径），`http-protocol.md` 修示例 + 指回。**不做本地兜底**：补出来的 `createdAt` 是本机拉取时刻，跨设备会各不相同。18 条纯函数 + 3 条走真 `upsertMany` 的用例，包内 1067 条全绿、覆盖率 96.3%/91.49%；下游 7 个适配器与 `dev-rxdb-supabase` 无回退。**D1 遗留问题已答**：子类重声明基类属性能把 `createdAt` 改成可空（`propertyMap.set` 是整条替换），但基类的 `readonly` / `default` 会一并消失且无提示——要变成推荐做法需另开故事。另留同族缺口一处：PGlite 的 `upsert_many_sql.ts` 已处理异构与未知键，未检查缺非空列
- ✅ [US-215 条件请求被静默停用时给出可观测信号](stories/adapter/US-215-conditional-request-silence.md) — 出自 US-214，2026-08-27 关闭。文档半边已由其 AC#12 补齐，缺的是给没读文档的人的信号：`#cacheAndReturn` 的 `etag === null` 分支本身正确，但「远端确实没发」与「跨源没暴露」在客户端完全重合，回调因此**只报事实不断言成因**（`Response.type` 原样透出，浏览器跨源为 `'cors'`、Node 手工构造的 `Response` 恒为 `'default'`——**线索而非判据**，两个取值分别由单元用例与 e2e 钉住，D1 的「未核实」就此有答案）。包内零 `console`、`HttpAdapterOptions` 通体 hook 形，故走可选 `onEtagUnreadable`。**AC#4「关着的开关不产生噪音」由结构保证**：回调挂在 transport 的 `conditional` 对象**里面**，开关关掉时整个对象不存在，回调根本不可达，不靠运行期再判一次。去重表与 LRU 同界、随 `disconnect()` 清空，且**不被 `delete()` 清掉**——读不到 ETag 的那一支每次都调 `delete()`，跟着清就把「一个 key 只报一次」退化成「每次都报」。`entityName` 刻意做成可选且**不参与任何判定**：按它决定走不走条件缓存等于把漏传诊断参数变成静默丢缓存，正是本故事要治的病。包内 350 条用例绿、覆盖率 99.81%/97.81%。**一处落地偏差**：技术笔记写的「不改包外运行时代码」与 AC#8 直接冲突（回调只能由 demo 配），取 AC#8，落成 `?diagnostics=1` 开关——默认关，好让 D3 要求的「未配回调时控制台零输出」那条 e2e 仍守着真正的默认行为
