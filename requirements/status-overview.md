...
| Story                                                                              | 当前进度                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [US-025 核心包子系统按插件边界外移](stories/core/US-025-core-plugin-extraction.md) | 阶段 A（门面轴注册表类型化 + 网关原地作用域化）、阶段 B（QueryCache 读路径外移为 `@aiao/rxdb-plugin-querycache`）与阶段 C（历史 / 撤销重做 / 分支外移为 `@aiao/rxdb-plugin-history`）已交付，A1～A4 / B1～B5 / C1～C6 全 ✅；C 实到 `version/` 整棵迁出（含推拉半区），核心留下 `system/system-repositories.ts` 与 `sync-contract/` 两批原语，公开面 452 → 460（+20 / −12，破坏性），十二处计划偏差记在故事的阶段 C 一节；阶段 D 两条前置齐备，是下一个可开工阶段——切割面改为**从历史插件里**切出 `@aiao/rxdb-plugin-sync` |
...
- 🚧 [US-025 核心包子系统按插件边界外移](stories/core/US-025-core-plugin-extraction.md) — QueryCache / 跨 tab 网关 / 历史分支 / 推拉同步 / 树实体分五阶段外移为插件包；阶段 A（门面轴注册表类型化 + 网关原地作用域化）、阶段 B（QueryCache 读路径外移，破坏性：`QueryCacheRepository` 退出公开面）与阶段 C（历史 / 分支外移，破坏性：`VersionManager` 等 12 条退出核心公开面）已交付；D～E 未开始，D 的两条前置已具备
