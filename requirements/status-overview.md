...
| [US-905 Tauri DevTools 调试窗口](stories/future/US-905-tauri-native-devtools.md)                                  | 阶段 1：**AC#1 已于 2026-09-04 关闭**——新建 `tauri-package-dev`（裸 `cargo build`，`cfg(dev)` 才成立）+ 自检报告 schema v3 + `devtools-smoke` target，实测 dev 产物窗口集合恰为 `["main", "rxdb-devtools"]`，且主窗口收到调试窗口经真实 Rust 中继送达的 `PROTOCOL_HELLO` + `HANDSHAKE_ACK`——US-905 第一次拿到**两个真实 WebView 完成 v2 握手**的证据。同批修掉三处缺陷：Rust 的 `emit` 实为广播（要 `emit_to`）、JS 全局 `listen` 的 `Any` target 无视投递过滤（要 `getCurrentWebviewWindow().listen`）、`build-devtools` 产物被 `build` 的 nx 缓存恢复抹掉（依赖反转 + 声明 `outputs`）。**AC#4 亦已关闭**：新增 dev-only 的窗口回收命令驱动「同 label 重开」，实测拿到**另一个** UUID v4 session；同批补上 Tauri 侧缺失的「对端没了」信号（Rust 发不带 payload 的讣告 → 页内 transport 翻译成 `DISCONNECT`），否则 connector 的 session 永不释放、重开的面板协商不上。**AC#3 亦已关闭**：开一扇 label 不在白名单里的真实窗口去敲中继，实测被拒（判据取拒绝计数而非布尔——0 说明根本没敲到门）。**AC#2/#5/#6 仍 ⚠️**，且**都不是纯写用例**：AC#2 要求逐例现装配会话（capability/时钟/协议版本逐例变化），真实窗口里的 connector 是固定档位的 bootstrap 单例复用不了，得在 demo 里再建一套 conformance bootstrap——往产品塞多少测试脚手架需 owner 划边界；**AC#5 亦已关闭**——修掉那条**镜像缺陷**（主窗口刷新后面板不重新协商）：面板端点在协商落定后再收到 legacy 握手即换新端点，落点在面板 library 而非阶段 B 冻结的协商机；实测三轮 session 两两不同；wa-sqlite 三档需阶段 2 或一个后端强制开关。另修掉「调试窗口从来没有样式」（vite 配置缺 tailwind + 无样式表入口）；阶段 2 未开工 |
