...
| Story                                                                                                             | 还剩什么                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [US-904 DevTools 原生本地存储调试](stories/future/US-904-devtools-native-storage-contract.md)                     | 阶段 A ✅、阶段 B ✅；阶段 C 主体已交付（C1 面板抽取 AC#31/32/33/35 ✅、C2 四段 relay v2 迁移 AC#36/37/41/43 ✅，AC#40/#44 ⚠️ 部分，AC#34/#38/#39/#42 ⬜ 待人工浏览器回归与跨版本实证）；阶段 D 已开工并基本落地——AC#45/#46/#48/#49/#50/#52/#53 ✅（AC#52 于 2026-09-03 真机跑通；AC#45/#46/#49 于 2026-09-04 关闭），**只剩 AC#47** ⚠️（面板侧下载未接线，见下方 epic 索引）                                                                                                                                                                                                                                                                                                                                                  |
| [US-905 Tauri DevTools 调试窗口](stories/future/US-905-tauri-native-devtools.md)                                  | 阶段 1：**AC#1 已于 2026-09-04 关闭**——新建 `tauri-package-dev`（裸 `cargo build`，`cfg(dev)` 才成立）+ 自检报告 schema v3 + `devtools-smoke` target，实测 dev 产物窗口集合恰为 `["main", "rxdb-devtools"]`，且主窗口收到调试窗口经真实 Rust 中继送达的 `PROTOCOL_HELLO` + `HANDSHAKE_ACK`——US-905 第一次拿到**两个真实 WebView 完成 v2 握手**的证据。同批修掉三处缺陷：Rust 的 `emit` 实为广播（要 `emit_to`）、JS 全局 `listen` 的 `Any` target 无视投递过滤（要 `getCurrentWebviewWindow().listen`）、`build-devtools` 产物被 `build` 的 nx 缓存恢复抹掉（依赖反转 + 声明 `outputs`）。**AC#2 仍 ⚠️**：那 80 条 conformance 今天跑在进程内 relay 上，不是这两个真实窗口之间，握手打通不等于它。AC#3～#6 未动；阶段 2 未开工 |
...
- 🚧 [US-904 DevTools 原生本地存储调试](stories/future/US-904-devtools-native-storage-contract.md)
  ...
  - 🚧 阶段 D Electron 原生存储集成（AC#45/#46/#48/#49/#50/#51/#52/#53 ✅；**只剩 AC#47** ⚠️）
    - **AC#49 已于 2026-09-04 关闭**：wire 侧强制调用经 connector 自己的 window 总线打进去，`settings.export` → `export_unsupported`（已声明但恒不支持）、未声明的 `settings.clear` → `provider_unsupported`（descriptor 层拒），两次调用前后库文件/WAL/SHM 的 mtime 与 WebView 自有存储清单逐项不变
    - **AC#51 已于 2026-09-04 关闭**：AC 的「关闭/刷新」两条路径都实证通过。「只关闭不刷新」那条最初红在**三处互相独立**的缺陷上，均已修：① 中继 `port.onDisconnect` 不通知页面，session 一直 open（现补发带真实 session 的 v2 `DISCONNECT`）；② `endpoint.ts` 的 `#route` 未跳过 `NEGOTIATION_OWNED_TYPES`，把新面板的 `PROTOCOL_HELLO` 回成 `session_invalid`；③ 协商机的 `sessionId` 构造时即铸死，光关 session 会让 B 复用 A 的身份（现改为整个换端点，与面板侧按 `connectionEpoch` 换端点对称）。三者任缺其一表征相同：面板静默退回 v1 车道而守卫仍显示「已连接」
    - **AC#46 已于 2026-09-04 关闭**：新增 demo 侧事件探针（走公开的 `RxDB.dispatchEvent()`）逐类派发 25 类，类型全集判据取 **wire**（面板的事件列表是 CDK 虚拟滚动，在 dock DevTools 里实测 `clientHeight` 为 0，DOM 只有三五条，不能当判据），面板是否收下取它自己的事件计数；branch 走「面板建/切 → 应用侧独立读数核对」
    - **AC#47 三项已补齐、只剩「面板侧下载」**：`runtime: electron` 显示（面板此前根本不显示 runtime，新增只读徽章 + 「换 runtime 除显示外逐项不变」单测，顺带解掉 US-905 AC#6/#10 的 UI 半边）、越限上传拒绝、失败无半写。剩下的那半**是产品侧没接线**：`v2-file-channel.ts` 的 `download()` 走普通请求路径，全仓库无人调用 `DevToolsPanelEndpoint.download()` 那条带 sink 的字节通道，所以字节从未到达面板；怎么把字节交给用户涉及 UX 决定（WebKit 上没有 `showSaveFilePicker`），**未在本轮实现**
