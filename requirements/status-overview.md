...
> 均为 `Backlog`）。父故事不直接交付，其状态在全部子故事 Done 后才随之关闭；因此 Backlog 16 中有 2 条是共享契约文档，
...
> [US-013](stories/core/US-013-effect-scope-primitive.md) / [US-014](stories/core/US-014-plugin-scope-contract.md)（均 Backlog）：
> 把仓库内已存在的九处「手工配对安装/拆卸」收敛成一个作用域原语，是把既有实现缺口登记成故事，不是新增产品范围；
> Backlog 14 → 16，合计 50 → 52。
