...
本节遗留的六个开项（AC1 未推送、AC6 缺用例、AC11 等 US-305、US-207 AC#2/AC#8、PR CI 接门禁、AC11 归属）
按 [`release-open-items-plan.md`](./release-open-items-plan.md) 执行。该方案逐条区分了
「分支里能关掉的」「Verdaccio 只是演练不构成依据的」「被前置阻塞的」与「需要拍板的」，
执行结果回写各 story 的 AC 表与本文件。
