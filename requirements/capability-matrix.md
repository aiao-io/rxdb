...
| HTTP                    | 待实现（US-212 定）              | `http`                  | Remote | **未实现**：远端权威 HTTP + 独立注册 sqlite 行缓存；v1 只支持 QueryCache，`pullChanges`/`mergeChanges`/`getChangeCount` throw unsupported | [US-212](stories/adapter/US-212-http-adapter.md)（Backlog；已排入 [roadmap 批次 1 线 F](roadmap.md)，**零前置**——[US-020](stories/core/US-020-querycache-repository.md) 两阶段已于 2026-08-22 全关，两档发布门禁同时解除） |
