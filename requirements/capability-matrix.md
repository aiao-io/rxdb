...
| HTTP                    | 待实现（US-212 定）              | `http`                  | Remote | **未实现**：远端权威 HTTP + 独立注册 sqlite 行缓存；v1 只支持 QueryCache，`pullChanges`/`mergeChanges`/`getChangeCount` throw unsupported | [US-212](stories/adapter/US-212-http-adapter.md)（Backlog；硬前置 [US-020](stories/core/US-020-querycache-repository.md)）                           |
