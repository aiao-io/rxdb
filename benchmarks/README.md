...
- `bench-working-tree`：工作树 `status` / `diff` / `restore` / `commit` 的 p95 ÷ 同次 control p95，≤ 冻结 reference median × 该项容差（读项 130%，写项 110%；PR CI 唯一硬门禁，契约见 `docs/working-tree/contracts/benchmark-report.md` §3）
