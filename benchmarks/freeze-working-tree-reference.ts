 ...
 * 这个脚本因此只在三种时刻跑：首次冻结；本画像测点集合发生变化（如 T109 加入 `restore`）后的
 * 重新冻结（`--regenerate`）；为一个还没有 reference 的画像冻结（`--new-profile`，所在提交须先在
 * 已冻结画像上过相对门禁）。候选版本没过门禁时，该动的是实现或者 plan.md 的例外，不是这里。
