...
KONG_PORT=$(grep -E '^KONG_HTTP_PORT=' .env 2>/dev/null | cut -d'=' -f2 || echo "8000")
...
# 等待 Kong 健康
counter=0
until curl -s "http://localhost:${KONG_PORT}/rest/v1/" > /dev/null 2>&1; do
    counter=$((counter + 1))
    if [ $counter -gt $timeout ]; then
...
echo "=========================================="
echo ""
echo "📍 API URL:        http://localhost:${KONG_PORT}"
