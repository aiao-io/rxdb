 ...
 * 只有容器形态不同 —— React 是渲染快照，Angular 是 `Signal`，Vue 是 `ComputedRef`。
 *
...
describe('useWorkingTree：loading 可观测（§4）', () => {
  it('命令发出后、结果回来之前，那一格是 loading', async () => {
    ...
    act(() => void (running = tree().isEnabled()));
  ...
  it('提交期间不静默：commitState 先进 loading', async () => {
    ...
    let running!: Promise<CommitResult>;
    act(() => void (running = tree().commit('第一次提交', { ...CREDENTIALS, authorId: 'alice', operationId: 'op-1' })));
  ...
  it('恢复期间不静默：restoreState 先进 loading', async () => {
    ...
    let running!: Promise<WorkingTreeRestoreResult>;
    act(() => void (running = tree().restore(RESTORE_TARGET, CREDENTIALS)));
  ...
  it('切换期间不静默：switchBranchState 先进 loading', async () => {
    ...
    act(() => void (running = tree().switchBranch('feature')));
...
describe('useWorkingTree：查询的 empty 相位（§4）', () => {
  ...
  it('零条目的 diff 落在 empty，有条目落在 success', async () => {
    ...
    await act(async () => void (await tree().diff()));
  ...
  it('没有历史的 listCommits 落在 empty', async () => {
    ...
    await act(async () => void (await tree().listCommits()));
  ...
  it('没有未结束会话时 restoreSession 落在 empty，并且照样带着那个 null', async () => {
    ...
    await act(async () => void (await tree().restoreSession()));
...
describe('useWorkingTree：不给无 empty 语义的命令伪造 empty（§4）', () => {
  ...
  it('零变更的 commit 落在 error 并把 empty_commit 原样抛出，不是 empty', async () => {
    ...
    await act(async () => {
      await expect(
        tree().commit('没东西可提交', { ...CREDENTIALS, authorId: 'alice', operationId: 'op-2' })
  ...
  it('discard 的 0 条是 no-op 结果，落在 success 而不是 empty', async () => {
    ...
    await act(async () => void (await tree().discard(CREDENTIALS)));
  ...
  it('CommitConflict 落在 success，不翻译成 error', async () => {
    ...
    await act(async () => {
      result = await tree().commit('并发提交', { ...CREDENTIALS, authorId: 'alice', operationId: 'op-3' });
    });
  ...
  it.each(REJECTED_RESTORES)('restore 被拒（$reason）落在 success，载荷原样带着', async rejected => {
    ...
    let result!: WorkingTreeRestoreResult;
    await act(async () => {
      result = await tree().restore(RESTORE_TARGET, CREDENTIALS);
    });
  ...
  it('restore 的 restoredCount 为 0 是 no-op 结果，落在 success 而不是 empty', async () => {
    ...
    await act(async () => void (await tree().restore(RESTORE_TARGET, CREDENTIALS)));
  ...
  it('切到当前分支是成功的 no-op，落在 success 而不是 empty', async () => {
    ...
    await act(async () => void (await tree().switchBranch('main')));
...
describe('useWorkingTree：改动之后 status 自己跟上', () => {
  ...
  it('一次成功的 commit 之后重读 status', async () => {
    ...
    workingTree.status.mockResolvedValue(statusWith(0));
    await act(
      async () => void (await tree().commit('提交', { ...CREDENTIALS, authorId: 'alice', operationId: 'op-4' }))
    );
    await flush();
  ...
  it('一次成功的 switchBranch 之后重读 status', async () => {
    ...
    workingTree.status.mockResolvedValue(statusWith(0));
    await act(async () => void (await tree().switchBranch('feature')));
    await flush();
...
describe('useWorkingTree：没有 provider', () => {
  ...
  it('抛错，不返回伪造的干净态', () => {
    expect(() => renderHook(() => useWorkingTree())).toThrow(/No RxDB instance found/);
  });
});
...
describe('tri-framework-api.md §3 清单守卫', () => {
  ...
  it('switchBranch 的第二参原样传给核心；不传就是不传', async () => {
    ...
    await act(async () => void (await tree().switchBranch('feature', options)));
    await act(async () => void (await tree().switchBranch('main')));
