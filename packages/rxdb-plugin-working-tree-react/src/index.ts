...
/**
 * rxdb-plugin-working-tree-react - React 集成层
 *
 * 提供 `useWorkingTree()` hook，把 `@aiao/rxdb-plugin-working-tree` 的 `WorkingTreeManager`
 * 适配成 React 状态消费接口。命名与 Angular / Vue 绑定层一致（tri-framework-api.md §3）。
 *
