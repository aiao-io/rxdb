...
vi.mock('../pull-repository.js', async importOriginal => ({
  ...(await importOriginal<typeof import('../pull-repository.js')>()),
...
// 推送侧的部分进度 —— 抹掉即导入失败。
vi.mock('../push-repository.js', async importOriginal => ({
  ...(await importOriginal<typeof import('../push-repository.js')>()),
  pushRepository: mocks.pushRepository
}));

// `getSyncType` 随 `sync-contract/` 留在了核心（US-025 阶段 C），而 `sync-repository.ts`
// 是从包根导入它的 —— 于是替身只能挂在包根上。铺开原模块再覆盖这一个符号，
// 其余导出（事件类、`RxDBError`……）保持同一份引用，`instanceof` 断言不受影响。
