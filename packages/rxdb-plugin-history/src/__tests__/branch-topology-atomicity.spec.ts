...
class FakeLocalDatabase {
  ...
  /**
   * 交给 `rxdb.localAdapter$` 的那一份。
   *
   * 除了 `transaction` / `removeMany`，还得答得出 `getRepository` —— US-025 阶段 C 之后
   * `getCurrentBranch` 是核心函数，经 `rxdb.localAdapter$` 自己解析系统表仓库，
   * 不再从 `VersionManager.getLocalRepositories()` 拿。交出的仍是**走队列**的那一份
   * （`direct = false`），冷路径的写照旧只能发生在事务内。
   */
  get adapter(): Pick<FakeLocalDatabase, 'transaction' | 'removeMany'> & {
    getRepository: (EntityType: EntityType) => unknown;
  } {
    return {
      transaction: fun => this.transaction(fun),
      removeMany: rows => this.removeMany(rows),
      getRepository: (EntityType: EntityType) => {
        ...
        throw new RxDBError(`假适配器没有 ${String(EntityType)} 的仓库`);
      }
    };
  }
