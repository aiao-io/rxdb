...
function createHarness(options: HarnessOptions = {}) {
  ...
  // 两种查询打在同一个 `find` 上：
  // - `activated = true` 是 `getCurrentBranch()` 的热路径（US-025 阶段 C 后，推送资格判定
  //   经核心的 `resolvePushIneligibility(rxdb, …)` 走到这儿，不再由替身直接返回分支）；
  // - `id = X` 是 `getAncestorBranchIds()` 沿 parentId 逐级上溯。
  // 热路径必须答得出来，否则 `getCurrentBranch` 会掉进冷路径去开事务建 main。
  const branchFind = vi.fn(async (findOptions: BranchFindOptions): Promise<RxDBBranch[]> => {
    if (ruleValue(findOptions.where, 'activated') === true) {
      return [createBranch(currentBranchId, branchParents[currentBranchId] ?? null)];
    }
  ...
  /** 只数祖先链上溯的那些查询，把「当前分支解析了几次」排除在外 */
  const ancestorLookups = () =>
    branchFind.mock.calls.filter(([findOptions]) => typeof ruleValue(findOptions.where, 'id') === 'string');
...
describe('pushRepository', () => {
  ...
  it('多分支查询使用所有祖先分支的最小水位线', async () => {
    ...
    expect(harness.ancestorLookups()).toHaveLength(3);
    expect(harness.changeFind).toHaveBeenCalledWith({
      where: {
        combinator: 'and',
        rules: [
