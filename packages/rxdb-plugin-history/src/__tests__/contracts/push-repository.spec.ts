...
function createValidationHarness(options: ValidationHarnessOptions = {}) {
  ...
  const branchRepo = createBranchRepositoryStub();
  // `getLocalSystemRepositories()` 一次解析三张系统表里的两张（branch + change），
  // 即便调用方只用得上分支——所以 change 也得答得出来，哪怕本用例一行都不读它。
  const changeRepo = { find: vi.fn(async () => [] as RxDBChange[]) } as unknown as IRepository<typeof RxDBChange>;
  const localAdapter = {
...
function createEmptyPushHarness() {
  ...
  const vm = {
    rxdb: {
      ...
      context: { clientId: 'contract-client' },
      dispatchEvent,
      // 同上：资格判定经核心解析系统表，走的是 `localAdapter$`。
      localAdapter$: of(localAdapter),
