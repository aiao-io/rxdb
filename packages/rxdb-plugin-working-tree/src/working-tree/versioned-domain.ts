/**
 * @fileoverview 版本化域（tracked / untracked）的**唯一**一份清单（epic-006「版本化域」、FR-046）。
 *
 * @remarks
 * raw 通道的 4 步判定、批量写门禁、四个捕获挂载点问的都是这一份。散成两份的代价不是不一致
 * 的编译错误，而是某个实体在一条写入口上进版本控制、在另一条上不进——而 epic-006「版本化域」
 * 第二条硬规则恰恰禁止这件事。
 *
