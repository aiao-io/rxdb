...
import { RXDB_CHANGE_VALUE_ENVELOPE_KEY } from '@aiao/rxdb';
import { describe, expect, it, vi } from 'vitest';
import type { CommitChangeUnitContent } from '../../commit/change-unit.js';
import type { CommitChangeSetRow, CommitPatchCodecContext } from '../../commit/commit-codec.js';
import {
...
import { codecWith, ENVELOPE, isEnvelopeLike, RAW_CIPHERTEXT, SENTINEL } from './fixtures/encrypted-entities.js';

/** 普通（未加密）二进制列的值，用来证明本断言不越界管它。 */
const BLOB = Uint8Array.of(0x01, 0x02, 0x03);

/** 没有判定器的上下文——只有「实体不含加密属性」时才允许走到这里。 */
const bareContext: CommitPatchCodecContext = codecWith();
...
describe('加密列必须以落库形态进入 commit（FR-038）', () => {
  it('信封形态原样通过——且判定器确实看过它，本组其余断言不是空跑', () => {
    ...
    expect(() => assertCommitUnitsEncryptedAtRest(codecWith(isEncryptedAtRest), [unit])).not.toThrow();
  ...
  it('明文落进加密列时拒绝——哪怕它是个合法字符串', () => {
    ...
    expect(() => assertCommitUnitsEncryptedAtRest(codecWith(isEnvelopeLike), [unit])).toThrow(
      CommitEncryptedAtRestError
    );
  });
  ...
  it('inversePatch 与 patch 同权——恢复数据泄漏和正向数据泄漏是同一件事', () => {
    ...
    const error = (() => {
      try {
        assertCommitUnitsEncryptedAtRest(codecWith(isEnvelopeLike), [unit]);
        return undefined;
  ...
  it('整批里只要有一条不合规就整批拒绝，错误指名到那一条', () => {
    ...
    const error = (() => {
      try {
        assertCommitUnitsEncryptedAtRest(codecWith(isEnvelopeLike), units);
        return undefined;
      } catch (caught: unknown) {
        return caught;
      }
