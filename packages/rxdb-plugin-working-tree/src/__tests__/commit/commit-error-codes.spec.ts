 ...
 *
 * 1. **码集会被悄悄扩张**。九个码分属四个故事，后续任何一个故事都可能顺手加一个
...
import { RxDBMixedVersionedCacheTransactionError } from '@aiao/rxdb';
import { describe, expect, it } from 'vitest';
import { COMMIT_ERROR_CODES, CommitErrorCode, isCommitErrorCode } from '../../commit/commit-error-codes.js';

describe('提交错误码', () => {
  it('码集恰好是契约 core-api.md §7 与 FR 点名的九条，不多不少', () => {
  ...
  it('isCommitErrorCode 认全部九条，拒未登记字符串与非字符串', () => {
    for (const code of COMMIT_ERROR_CODES) {
      expect(isCommitErrorCode(code)).toBe(true);
    }
    // `commit_conflict` 永远不进码集：core-api.md §7 明确它是返回值不是异常。
