/**
 * @fileoverview 受信调用点的**结论列**：epic-006「受信调用点登记表」第 7 列「产生工作树单元」（SC-010）。
 *
 * @remarks
 * 那张表的另外六列描述的是核心的事实——哪个文件、哪个符号、哪个写原语、什么意图——
 ...
 */
const CONTRACT_PRODUCES_ENTRY: readonly boolean[] = sectionOf(EPIC_MARKDOWN, '#### 受信调用点登记表', '\n### ')
  .split('\n')
