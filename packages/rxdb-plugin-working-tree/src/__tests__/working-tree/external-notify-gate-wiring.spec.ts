...
import type { RxDB, RxDBEvent, WorkingTreeCaptureHook } from '@aiao/rxdb';
import { Entity, EntityBase, EntityManager, PropertyType, SyncType } from '@aiao/rxdb';
...
/**
 * 钩子替身；转交门按真运行时的写法接上判定
 *
 * @returns 一个只够本文件跑起来的捕获钩子
 *
 * @remarks
 * `gateExternalNotify` **不写成「按参数返回预设结论」**：核心侧交出去的只剩实体名与命名空间，
 * 归类与判定都在钩子这一侧。替身若自己编一个结论，本文件断的就是替身而不是「行 11 有没有被接上」。
 * 所以这里照真运行时的形状走一遍：先归类，再问那一份门禁。
 */
const hookStub = (): WorkingTreeCaptureHook => {
  const domain = domainStub();
  const targetClassOf = (entityName: string, namespace?: string): WriteTargetClass =>
    namespace === 'rxdb' ? 'system'
    : domain.classifyEntity(entityName) === 'untracked' ? 'query_cache'
    : 'versioned';
  return {
    gateExternalNotify: (entityName, namespace, notify) =>
      gateExternalNotify(
        { entityName, targetClass: targetClassOf(entityName, namespace), capabilityEnabled: true },
        notify
      ),
    // raw 通道不在本文件的射程里；接上是为了让替身满足整张契约，而不是留个会炸的洞。
    gateRawWrite: async (_sql, execute) => await execute(),
