/**
 * @fileoverview 本插件持久层的单条引导迁移（data-model.md §8）
 *
 * @remarks
 * epic-006「本地工作树与提交历史」的持久层一次性到位。**只有这一条**：
 * 10 张表的初始行分散成多条迁移，就没有任何一处能保证「要么全有要么全无」。
 *
 * 它随本包走、经 `RxDBSystemContribution.createMigrations()` 交给核心，与核心自带的
 * 系统迁移并进**同一条**链、同一张 `rxdb_migration`、同一把锁。名字里的 `0004-` 是发布时
 * 就钉死的历史编号（那时这条迁移还长在核心里），不是本包自己的序号——见
 * {@link WORKING_TREE_COMMITS_MIGRATION_NAME}。
 */
 ...
 *
 * **表不在这里建**。10 张表经 `RxDBSystemContribution.entities` 登记进核心的系统实体
 * 注册表，由 `createTables()`（新库）与 `RxDB.connect()` 的系统表补建（既有库）建出，
 * 六个后端各自的建表器负责方言。两个后端的
