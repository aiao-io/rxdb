...
/** 最近一次构造的表格实例（React 测试无法直接拿组件内部实例，经此全局句柄驱动事件）。 */
let lastInstance: FakeListTable | null = null;

/** 取最近构造的 {@link FakeListTable} 实例。 */
export const getLastListTable = (): FakeListTable => {
  if (!lastInstance) throw new Error('No FakeListTable has been created yet');
  return lastInstance;
};
 ...
 */
export const register: { editor: (name: string, editor: unknown) => void } = { editor: vi.fn() };
