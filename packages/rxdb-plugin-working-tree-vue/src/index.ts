/**
 * rxdb-plugin-working-tree-vue - Vue 集成层
 *
 * 提供 `useWorkingTree()` composable，把 `@aiao/rxdb-plugin-working-tree` 的 `WorkingTreeManager`
 * 适配成 Vue `ComputedRef` 消费接口。命名与 Angular / React 绑定层一致（tri-framework-api.md §3）。
 *
