...
/**
 * 本文件挂过的全部组件；`afterEach` 逐个卸载。
 *
 * @remarks
 * `@vue/test-utils` 的 `mount()` **不自带**自动卸载：丢掉 wrapper 就等于让这个组件连同它的
 * watcher 一直挂在 document 上，整份文件跑完时几十个实例仍然活着，前面用例的 watcher 还会
 * 对后面用例的 stub 调用产生反应。React 那侧是 `afterEach(cleanup)`、Angular 那侧是
 * `afterEach(() => TestBed.resetTestingModule())`，三端在这件事上必须一致。
 */
...
const createFixture = () => {
  ...
  return { workingTree, versionManager, tree: mountWithProvider(rxdb) };
};
...
describe('useWorkingTree：初始状态', () => {
  ...
  it('十二格全是 idle，创建入口本身一次 IO 都不发', () => {
    ...
    expect([
      ...
      tree.enableIfEmptyState.value.phase,
      tree.statusState.value.phase,
      tree.diffState.value.phase,
      tree.listCommitsState.value.phase,
      tree.commitChangesState.value.phase,
      tree.commitState.value.phase,
      tree.discardState.value.phase,
      tree.restoreState.value.phase,
      tree.restoreSessionState.value.phase,
      tree.switchBranchState.value.phase
...
describe('useWorkingTree：loading 可观测（§4）', () => {
  ...
  it('恢复期间不静默：restoreState 先进 loading', async () => {
    ...
    expect(tree.restoreState.value.phase).toBe('loading');

    pending.resolve(RESTORE_OK);
    await running;
    expect(tree.restoreState.value.phase).toBe('success');
  });
  ...
  it('切换期间不静默：switchBranchState 先进 loading', async () => {
    ...
    expect(tree.switchBranchState.value.phase).toBe('loading');

    pending.resolve();
    await running;
    expect(tree.switchBranchState.value.phase).toBe('success');
  });
});
...
describe('useWorkingTree：查询的 empty 相位（§4）', () => {
  ...
  it('没有未结束会话时 restoreSession 落在 empty，并且照样带着那个 null', async () => {
    ...
    expect(tree.restoreSessionState.value).toEqual({ phase: 'empty', value: null });
  });
  ...
  it('conflicted 的会话落在 success，不因为「不健康」被算成空', async () => {
    ...
    expect(tree.restoreSessionState.value).toEqual({ phase: 'success', value: sessionWith('conflicted') });
  });
});
...
describe('useWorkingTree：不给无 empty 语义的命令伪造 empty（§4）', () => {
  ...
  it.each(REJECTED_RESTORES)('restore 被拒（$reason）落在 success，载荷原样带着', async rejected => {
    ...
    expect(tree.restoreState.value).toEqual({ phase: 'success', value: rejected });
  });
  ...
  it('restore 的 restoredCount 为 0 是 no-op 结果，落在 success 而不是 empty', async () => {
    ...
    expect(tree.restoreState.value).toEqual({ phase: 'success', value: noop });
  });
  ...
  it('切到当前分支是成功的 no-op，落在 success 而不是 empty', async () => {
    ...
    expect(tree.switchBranchState.value).toEqual({ phase: 'success', value: undefined });
  });
  ...
  it('requireClean 撞上脏工作树时落在 error，并把 WorkingTreeDirtyError 原样抛出', async () => {
    ...
    expect(tree.switchBranchState.value).toEqual({ phase: 'error', error: failure });
  });
});
...
describe('useWorkingTree：改动之后 status 自己跟上', () => {
  ...
  it('restore 之后不顺手重读会话，restoreSessionState 停在 idle', async () => {
    ...
    expect(tree.restoreSessionState.value.phase).toBe('idle');
  });
