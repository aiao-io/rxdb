...
describe('electron settings provider — descriptor', () => {
  ...
  it('MUST take no host ports at all', () => {
    // 收一个 ports 入参就等于握着句柄；没有句柄，也就没有「顺手读一下」的可能。
    expect(createDevToolsElectronSettingsProvider).toHaveLength(0);
  });
});

describe('electron settings provider — AC#49', () => {
  it('MUST answer export_unsupported without reading SQLite, WAL or any directory', async () => {
    const probe = installHostProbe();
    const provider = createDevToolsElectronSettingsProvider();
  ...
  it('MUST answer identically to the browser provider for every operation', async () => {
    const electron = createDevToolsElectronSettingsProvider();
