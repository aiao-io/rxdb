...
```bash
pnpm add @aiao/rxdb @aiao/rxdb-plugin-querycache
```
...
漏掉 `use()` 时 `connect()` 抛 `RxDBMissingPluginError`，消息点名是哪个实体声明了 `SyncType.QueryCache`、要装哪个包。它**不会**降级成本地读——降级之后调用方看到的是「远端没有这行数据」，比直接失败难查得多。
...
- **离线**：远端不可达时读回落到本地缓存，写落本地并进出站队列，由 core 的写回路径在恢复后重放。
