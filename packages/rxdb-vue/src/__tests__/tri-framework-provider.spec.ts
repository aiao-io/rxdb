/**
 * 三端 provider 契约 —— Vue 侧。
 *
 * @remarks
 * 断言全部在**编译期**成立：`@ts-expect-error` 由 `nx typecheck rxdb-vue`
 ...
 *
 * Vue 在共享契约之外还收 `Ref`，那是本端的超集（{@link RxDBInput}），
 * 由 `public-types.spec.ts` 单独覆盖，不属于三端对称的部分。
 *
 * 对应文件（同名同结构，三端逐条对齐）：
 * - `packages/rxdb-angular/src/__tests__/tri-framework-provider.spec.ts`
 * - `packages/rxdb-react/src/__tests__/tri-framework-provider.spec.ts`
 */
import { RxDB } from '@aiao/rxdb';
import { describe, expect, it } from 'vitest';
import { provideRxDB, useRxDB, useRxDBOptional, type RxDBInput, type RxDBSource } from '../index';
...
describe('三端 provider 契约', () => {
  ...
  describe('provider 入口接受同一个联合类型', () => {
    it('provideRxDB 收 RxDBSource', () => {
      const accept = (source: RxDBSource<RxDB>): void => provideRxDB(source);
      // Vue 端超集：RxDBSource 是 RxDBInput 的子集，Ref 与 undefined 是本端独有的形态
      const widen = (source: RxDBSource<RxDB>): RxDBInput<RxDB> => source;

      expect([accept, widen]).toHaveLength(2);
    });
  });
});
