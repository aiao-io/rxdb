 ...
 * `requirements/api-baseline/rxdb-react.json` 的空 diff 承担，本文件守运行时导出。
 *
 * 对应文件（同名同结构，三端逐条对齐）：
 * - `packages/rxdb-angular/src/__tests__/tri-framework-field-descriptor.spec.ts`
