/**
 * 三端泛型契约（RAN-014）—— React 侧。
 *
 * @remarks
 * 断言全部在**编译期**成立：`@ts-expect-error` 由 `nx typecheck rxdb-react` 校验 ——
 ...
 * - `packages/rxdb-vue/src/__tests__/tri-framework-generics.spec.ts`
 */
...
import type { GraphPath, GraphQueryResult, NeighborResult, RxDBResource } from '../index.js';
import type { UseOptions } from '../query-options';
...
describe('三端泛型契约（RAN-014）', () => {
  ...
  describe('无限滚动选项遵循实体声明的 findByCursorOptions', () => {
    it('通用 FindByCursorOptions 形状被编译期拒绝', () => {
      const options: UseOptions<EntityStaticType<typeof DeclaredCursorEntity, 'findByCursorOptions'>> =
    ...
    it('实体声明的形状被接受', () => {
      ...
      const factory: UseOptions<EntityStaticType<typeof DeclaredCursorEntity, 'findByCursorOptions'>> = () => ({
        bucket: 'inbox'
      });
