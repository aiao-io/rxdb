...
describe('useInfiniteScroll', () => {
  ...
  describe('live cursor boundary (RRE-005)', () => {
    ...
    const createLiveDataset = (initial: CursorEntity[]): BehaviorSubject<CursorEntity[]> => {
      ...
      findByCursor.mockImplementation(options => rows$.pipe(map(rows => sqlPage(rows, options.after, options.limit))));
      return rows$;
    };
    ...
    const createMergingDataset = (initial: CursorEntity[]) => {
      let rows = initial;
      const opened: { cursor: CursorEntity | undefined; limit: number; page$: BehaviorSubject<CursorEntity[]> }[] = [];
      findByCursor.mockImplementation(options => {
        const { limit } = options;
        if (limit === undefined) throw new Error('本用例组的游标查询必须显式带 limit');
        const page$ = new BehaviorSubject<CursorEntity[]>(sqlPage(rows, options.after, limit));
        opened.push({ cursor: options.after, limit, page$ });
        return page$;
      });
      return {
        create: (...created: CursorEntity[]) => {
          rows = [...rows, ...created];
          opened.forEach(page =>
    ...
    it('keeps the tail anchored when an incremental head insert grows the first page', async () => {
      ...
      act(() => dataset.create(row('x', 0)));

      expect(result.current.value.map(entity => entity.id)).toEqual(['x', 'a', 'b', 'c', 'd']);
