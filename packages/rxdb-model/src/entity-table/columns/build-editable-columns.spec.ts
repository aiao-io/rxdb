...
import { buildEditableColumns, normalizeKVSchemaEntryType } from './build-editable-columns.js';
...
describe('buildEditableColumns', () => {
  ...
  it('builds property columns in propertyMap order with display names', () => {
    const meta = makeMeta({
      propertyMap: new Map([
        ['name', { name: 'name', type: PropertyType.string, displayName: '名称' }],
        ['age', { name: 'age', type: PropertyType.integer, displayName: '年龄' }]
      ])
    });
  ...
  it('skips id / createdAt / updatedAt / createdBy / updatedBy system fields', () => {
    const meta = makeMeta({
      propertyMap: new Map([
        ['id', { name: 'id', type: PropertyType.uuid }],
        ['createdAt', { name: 'createdAt', type: PropertyType.date }],
        ['updatedAt', { name: 'updatedAt', type: PropertyType.date }],
        ['createdBy', { name: 'createdBy', type: PropertyType.uuid }],
        ['updatedBy', { name: 'updatedBy', type: PropertyType.uuid }],
        ['visible', { name: 'visible', type: PropertyType.string }]
      ])
    });
    const columns = buildEditableColumns(meta);
  ...
  it('marks property-level readonly columns as non-editable', () => {
    const meta = makeMeta({
      propertyMap: new Map([['locked', { name: 'locked', type: PropertyType.string, readonly: true }]])
    });
    const columns = buildEditableColumns(meta);
  ...
  it('passes enum values into the enum editor column', () => {
    const meta = makeMeta({
      propertyMap: new Map([['status', { name: 'status', type: PropertyType.enum, enum: ['a', 'b'] }]])
    });
    const columns = buildEditableColumns(meta);
    const col = columns[1] as Record<string, unknown>;
  ...
  it('appends computed property columns after regular properties', () => {
    const meta = makeMeta({
      propertyMap: new Map([['name', { name: 'name', type: PropertyType.string }]]),
      computedPropertyMap: new Map([['fullName', { name: 'fullName', type: PropertyType.string, displayName: '全名' }]])
    });
    const columns = buildEditableColumns(meta);
