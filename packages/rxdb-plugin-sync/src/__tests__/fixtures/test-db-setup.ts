 ...
 *
 * ## 与核心 / 历史包那两份的差别（US-025 阶段 C、D）
 *
 * 本包这份在 `init()` 之前多两句 `use()`：`rxDBPluginHistory` 与 `rxDBPluginSync`。
 * 推拉同步搬进插件之后 `rxdb.syncManager` 不再由核心装配，而同步插件
 * `inject: ['plugin:history']` —— 少装历史那一句，同步插件整个不会安装，槽位也就不存在。
 * 三份各自成立、不是复制粘贴的遗留：核心那份**有意一个插件都不装**，守的是「不装插件时
 * 核心照样跑得起来」；历史包那份只装历史，守的是「没有同步也能撤销重做」。
 *
...
export async function createTestDB(options: TestDBOptions = {}): Promise<{
  ...
  // 顺序随意：宿主按 `inject` 拓扑排序，历史插件一定先于同步插件装完。
