...
function createHarness(options: HarnessOptions = {}) {
  ...
  // 两种查询打在同一个 `find` 上：`activated = true` 是 `getCurrentBranch()` 的热路径
  //（阶段 C 后拉取资格判定经核心的 `resolvePullIneligibility(rxdb, …)` 走到这儿），
  // `id = X` 才是 `getAncestorBranchIds()` 的逐级上溯。热路径答不出来就会掉进冷路径去开事务建 main。
  const branchFind = vi.fn(async (query: unknown): Promise<RxDBBranch[]> => {
    if (readRuleValue(query, 'activated') === true) {
      const id = options.currentBranchId ?? 'main';
