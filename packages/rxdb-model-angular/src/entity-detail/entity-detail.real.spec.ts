...
import { EntityDetailComponent, type EntityDetailDialogData } from './entity-detail';
...
describe('EntityDetailComponent（真实组件）', () => {
  ...
  it('关系 tabs：基本信息 + 一对多表 tab，切 tab 前校验草稿', () => {
    ...
    expect(component.tabs().map(t => t.key)).toEqual(['basic', 'children']);
  ...
  it('buildFixedQuery / buildFixedFormData 按关系类型生成固定条件', () => {
    const { component } = createWithDialog(dialogData());
    const tab = buildDetailTabs(getEntityMetadata(DetailGroup)).find(t => t.key === 'children')! as Parameters<
      EntityDetailComponent['buildFixedQuery']
    >[0];
  ...
  it('creationChain 阻断循环创建：链上实体不再出表 tab', () => {
    ...
    // 子实体已在链上 → 对应表 tab 被过滤
    expect(component.tabs().map(t => t.key)).toEqual(['basic']);
  });
