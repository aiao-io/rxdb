...
export class InMemoryRepository<T extends EntityType> implements IRepository<T> {
  ...
  find(options: {
    ...
    let rows = [...this.#table.values()].filter(row => {
      ...
      return isEntityMatchWhere(row, options.where as never);
    });
    if (options.orderBy?.length) {
      rows = calculateOrderBy(
        rows as never,
        options.orderBy as Array<{ field: string; sort: 'asc' | 'desc' }>
      ) as unknown as Row[];
    }
    if (options.offset) rows = rows.slice(options.offset);
    if (options.limit != null) rows = rows.slice(0, options.limit);
    return Promise.resolve(rows.map(row => this.#instantiate(row)));
  }

  count(options: { where?: { combinator: 'and' | 'or'; rules: unknown[] } }): Promise<number> {
  ...
  create(entity: EntityInstanceType<T>): Promise<EntityInstanceType<T>> {
  ...
  update(entity: EntityInstanceType<T>, patch: Partial<EntityInstanceType<T>>): Promise<EntityInstanceType<T>> {
    const meta = getEntityMetadata(this.EntityType);
    const id = String(snapshot(entity)['id']);
    ...
    getEntityStatus(entity).applyExternal({ ...changed } as never);
    const status = getEntityStatus(entity);
    status.origin = structuredClone(snapshot(entity));
    status.modified = false;
    status.local = true;
    return Promise.resolve(entity);
  }

  remove(entity: EntityInstanceType<T>): Promise<EntityInstanceType<T>> {
    const meta = getEntityMetadata(this.EntityType);
    const id = String(snapshot(entity)['id']);
    const stored = this.#table.get(id);
    if (stored) {
...
export class InMemoryRxDBAdapter extends RxDBAdapterLocalBase implements IRxDBAdapter {
  ...
  /** 按 SwitchVersionActions 键（`ns:entity:id` / `rxdb:RxDBChange:id`）应用回放动作。 */
  async #applyActions(actions: SwitchVersionActions): Promise<void> {
    ...
    for (const [key, action] of actions.deletes) {
      const [namespace, entity, id] = key.split(':');
  ...
  async createTables(EntityTypes: EntityType[], entities?: EntityInstanceType<EntityType>[]): Promise<boolean> {
    ...
    for (const entity of entities ?? []) {
      const meta = getEntityMetadata(entity.constructor as EntityType);
  ...
  async saveMany<T extends EntityType>(entities: EntityInstanceType<T>[]): Promise<EntityInstanceType<T>[]> {
    for (const entity of entities) {
      const table = this.tableFor(entity.constructor as T);
      const id = String(snapshot(entity)['id']);
      if (table.has(id)) await this.getRepository<T>(entity.constructor as T).update(entity, snapshot(entity));
      else await this.getRepository<T>(entity.constructor as T).create(entity);
    }
    return entities;
  }

  async removeMany<T extends EntityType>(entities: EntityInstanceType<T>[]): Promise<EntityInstanceType<T>[]> {
    for (const entity of entities) {
      await this.getRepository<T>(entity.constructor as T).remove(entity);
    }
    return entities;
  }

  async mutations<T extends EntityType>(options: RxDBMutationsMap<T>): Promise<EntityInstanceType<T>[]> {
    const all = new Set<EntityInstanceType<T>>();
    ...
    for (const [EntityType, entities] of options.update) {
      for (const entity of entities) {
        ...
        await this.getRepository<T>(EntityType).update(entity, patch as Partial<EntityInstanceType<T>>);
      }
    }
