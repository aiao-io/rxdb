...
import PopoverSelect from '../../../query-builder/popover-select/PopoverSelect.vue';

/**
 * PopoverSelect —— **真实加载组件源码**（对齐 Angular 侧）。
 *
 * 覆盖过滤 / 键盘导航 / Popover API 生命周期 / 定位计算的每一条分支。
 * happy-dom 无完整 Popover API，`hidePopover` / `showPopover` 打桩；
 * `focus` 走 happy-dom 真实实现（断言 `document.activeElement`）。
 */
...
type PopoverSelectVM = InstanceType<typeof PopoverSelect> & {
  uid: string;
  currentLabel: string;
...
describe('PopoverSelect（真实组件）', () => {
  function render(
    ...
    const wrapper = mount(PopoverSelect, {
      props: {
        options: inputs.options ?? [...OPTIONS],
        selected: inputs.selected ?? '',
        ...(inputs.placeholder ? { placeholder: inputs.placeholder } : {}),
        ...(inputs.minWidth ? { minWidth: inputs.minWidth } : {})
      },
      // happy-dom 的 focus 需要真实挂进 document 才更新 activeElement
      attachTo: document.body
    });
    const component = wrapper.vm as unknown as PopoverSelectVM;
    ...
    return { wrapper, component, el, popover };
  }
  ...
  describe('渲染与选项', () => {
    ...
    it('activeItemId 跟随高亮索引，越界时为空', () => {
      ...
      // 直接改 ref：经实例代理写回
      (host.wrapper.vm as unknown as { activeIndex: number }).activeIndex = 2;
      expect(component.activeItemId).toBe(`${component.uid}-opt-gamma`);

      (host.wrapper.vm as unknown as { activeIndex: number }).activeIndex = 99;
      expect(component.activeItemId).toBeNull();
    });
  });

  describe('搜索过滤', () => {
    it('过滤词大小写不敏感、前后空白裁剪', async () => {
      (host.wrapper.vm as unknown as { filterText: string }).filterText = '  bEtA ';
    ...
    it('空过滤词返回全部选项', async () => {
      ...
      (host.wrapper.vm as unknown as { filterText: string }).filterText = '   ';
      await host.wrapper.vm.$nextTick();
      expect(host.component.filteredOptions.length).toBe(3);
    });

    it('过滤无命中时渲染「无匹配项」空态，activeItemId 为 null', async () => {
      (host.wrapper.vm as unknown as { filterText: string }).filterText = 'zzz-no-match';
      await host.wrapper.vm.$nextTick();
    ...
    it('过滤词变化后高亮自动回到第一项', async () => {
      (host.wrapper.vm as unknown as { activeIndex: number }).activeIndex = 2;
      (host.wrapper.vm as unknown as { filterText: string }).filterText = 'be';
      await host.wrapper.vm.$nextTick();

      expect((host.wrapper.vm as unknown as { activeIndex: number }).activeIndex).toBe(0);
  ...
  describe('点击选择', () => {
    it('点击选项 emit selectChange、关闭弹层并聚焦触发按钮', () => {
      ...
      expect(host.wrapper.emitted('selectChange')).toEqual([['gamma']]);
  ...
  describe('键盘导航', () => {
    it('ArrowDown / ArrowUp 循环移动高亮并滚动可见项', async () => {
      host.component.onKeydown(new KeyboardEvent('keydown', { key: 'ArrowDown', cancelable: true }));
      expect((host.wrapper.vm as unknown as { activeIndex: number }).activeIndex).toBe(1);
      await host.wrapper.vm.$nextTick();
      ...
      // 顶部再上 → 环绕到最后一项
      host.component.onKeydown(new KeyboardEvent('keydown', { key: 'ArrowUp', cancelable: true }));
      expect((host.wrapper.vm as unknown as { activeIndex: number }).activeIndex).toBe(2);
    });
    ...
    it('Enter 选择当前高亮项', () => {
      ...
      expect(host.wrapper.emitted('selectChange')).toEqual([['beta']]);
      expect(host.popover.hidePopover).toHaveBeenCalledTimes(1);
    });
    ...
    it('无匹配选项时方向键与 Enter 安全 no-op', async () => {
      (host.wrapper.vm as unknown as { filterText: string }).filterText = 'zzz-no-match';
      await host.wrapper.vm.$nextTick();
  ...
  describe('Popover API 生命周期', () => {
    it('beforetoggle open 清空过滤词并应用弹层定位', async () => {
      (host.wrapper.vm as unknown as { filterText: string }).filterText = 'be';
    ...
    it('beforetoggle closed 不清空过滤词', async () => {
      (host.wrapper.vm as unknown as { filterText: string }).filterText = 'be';
...
describe('PopoverSelect（分支收尾）', () => {
  it('键盘滚动：目标选项已脱离文档时 scrollIntoView 安全跳过', () => {
    ...
    const popover = (wrapper.element as HTMLElement).querySelector('[popover]') as HTMLElement;
    popover.hidePopover = vi.fn();

    // 把当前高亮的下一项从文档摘除，模拟选项脱离 DOM 的场景
    (wrapper.element as HTMLElement).querySelector(`#${component.uid}-opt-beta`)?.remove();
