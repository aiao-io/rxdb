...
import { mountWithRxDB } from '../testing/mount-utils';
...
type ListVM = InstanceType<typeof EntityList> & {
  ...
  canUndo: boolean;
  canRedo: boolean;
  pendingQuery: { combinator: 'and' | 'or'; rules: unknown[] };
  pendingQueryValid: boolean;
  filterQuery: { combinator: 'and' | 'or'; rules: unknown[] };
  selectedCount: number;
  m2mAlreadyLinkedIds: Set<string>;
  tableRecords: EntityTableRecord[];
  tableColumns: Array<{ field?: string; title?: string }>;
  createFormFieldConfigs: unknown[];
  queryBuilderFields: unknown[];
  loadMore: (() => void) | undefined;
  sortState: { field: string; order: string };
  toggleFilterPopover(): void;
  applyFilter(): void;
  resetFilter(): void;
  onQueryChange(query: { combinator: 'and' | 'or'; rules: unknown[] } | undefined): void;
  onValidationChange(result: { valid: boolean }): void;
  onCellChanged(event: { col: number; row: number; field: string; value: unknown; record: EntityTableRecord }): void;
  onBatchUpdated(mutations: Array<{ recordId: string; changes: Record<string, unknown> }>): void;
  onIconClicked(event: { name: string; record: EntityTableRecord }): Promise<void>;
  onRowDeleted(event: EntityTableRecord): Promise<void>;
  onSortClicked(event: { field: unknown; order: unknown }): void;
  undo(): void;
  redo(): void;
  openCreateDialog(): void;
  openM2mSelectDialog(): void;
  onM2mSelectionConfirmed(entities: unknown[]): void;
 ...
 * 撤销/重做（真实 @aiao/rxdb-plugin-history）、筛选弹层（内置 QueryBuilder）、
...
describe('EntityList（真实组件）', () => {
  ...
  async function renderList(inputs: Partial<{ namespace: string; name: string; mode: 'default' | 'select' }> = {}) {
    const { wrapper, child } = mountWithRxDB(EntityList, rxdb, {
      props: {
        namespace: inputs.namespace ?? 'public',
        name: inputs.name ?? 'Todo',
        ...(inputs.mode ? { mode: inputs.mode } : {})
      }
    });
    mounted.push(wrapper);
    const component = child.vm as unknown as ListVM;
    await FLUSH();
    return {
      wrapper,
      child,
      component,
      confirmed: () => ((child.emitted('selectionConfirmed') as unknown as unknown[][]) ?? []).map(e => e[0]),
      cancelled: () => (child.emitted('selectionCancelled') as unknown as unknown[]) ?? [],
      viewed: () => ((child.emitted('viewEntity') as unknown as unknown[][]) ?? []).map(e => e[0])
    };
  }

  /** 列表渲染树里的真实表格引擎实例（entity-list → query-table → entity-table）。 */
  function tableOf(child: ReturnType<typeof mountWithRxDB>['child']): FakeListTable {
    return child.findComponent(QueryTable).vm.tableInstance as unknown as FakeListTable;
  }
  ...
  it('namespace/name 输入驱动 useInfiniteScroll 加载真实数据', async () => {
    ...
    expect(wrapper.element.textContent).toContain('Todo');
    expect(component.tableRecords).toHaveLength(2);
    const titles = component.tableRecords.map(r => r['title']).sort();
  ...
  it('行内编辑经真实事件链（VTable → cellChanged → mutations）批量合并落库', async () => {
    ...
    // 真实事件链：表格引擎事件 → EntityTable → QueryTable → 本组件
  ...
  it('筛选弹层：query-builder 条件应用后列表按 where 重查', async () => {
    ...
    // 打开弹层 → Teleport 渲染真实 QueryBuilder
    ...
    expect(component.tableRecords.map(r => r['title']).sort()).toEqual(['match-alpha', 'match-beta']);
  ...
  it('选择模式：勾选后 selectionConfirmed 输出真实实体实例', async () => {
    ...
    expect((confirmed()[0] as Array<{ id: string }>).map(e => e.id)).toEqual([a.id]);
  });
  ...
  it('view-action 懒加载对话框在组件销毁后不再打开（防 NG0205 竞态）', async () => {
    ...
    const { wrapper, child, component, viewed } = await renderList();
    await FLUSH();
    const record = component.tableRecords[0];

    const openPromise = component.onIconClicked({ name: 'view-action', record });
    // emit 在 onIconClicked 内同步发生；unmount 会清空 test-utils 的事件记录，先取快照
    const viewedSnapshot = viewed();
    wrapper.unmount();
    await openPromise;
    await FLUSH();
    await FLUSH();

    expect(viewedSnapshot).toEqual([record]);
  ...
  it('Ctrl+Z / Ctrl+Shift+Z 键盘快捷键驱动 undo / redo', async () => {
    ...
    wrapper.element.dispatchEvent(new KeyboardEvent('keydown', { key: 'z', ctrlKey: true, cancelable: true }));
    await vi.waitFor(() => {
    ...
    wrapper.element.dispatchEvent(inputEvent);
  ...
  it('creationChain 含当前实体时阻断新增（按钮隐藏、openCreateDialog 为 no-op）', async () => {
    ...
    const { wrapper, child } = mountWithRxDB(EntityList, rxdb, {
      props: { namespace: 'public', name: 'Todo', creationChain: ['public:Todo'] }
    });
    mounted.push(wrapper);
    const component = child.vm as unknown as ListVM;
    await FLUSH();
  ...
  it('initialFilter 合法 JSON 载入初始筛选，非法 JSON 静默忽略', async () => {
    ...
    // 非法 JSON：保持空筛选且不抛错
    const { wrapper: badWrapper, child: badChild } = mountWithRxDB(EntityList, rxdb, {
      props: { namespace: 'public', name: 'Todo', initialFilter: '{not-json' }
    });
    mounted.push(badWrapper);
    const badComponent = badChild.vm as unknown as ListVM;
    await FLUSH();
    await FLUSH();
  ...
  it('fixedQuery 单独生效，与用户筛选 AND 合并', async () => {
    ...
    // 仅固定条件（用户筛选为空）
    await vi.waitFor(() => {
      expect(component.tableRecords.map(r => r['title']).sort()).toEqual(['fixed-a', 'fixed-b']);
    });
  ...
  it('未知实体：displayName 回退 name、列回退 actionsColumn、编辑与新增全部安全 no-op', async () => {
    ...
    const { wrapper, child } = mountWithRxDB(EntityList, rxdb, {
      props: { namespace: 'public', name: 'Ghost' }
    });
    mounted.push(wrapper);
    const component = child.vm as unknown as ListVM;
    await FLUSH();
  ...
  it('MANY_TO_MANY 模式渲染「+ 添加」按钮并暴露 isM2m', async () => {
    ...
    const { wrapper, child } = mountWithRxDB(EntityList, rxdb, {
      props: { namespace: 'public', name: 'Todo', relationKind: RelationKind.MANY_TO_MANY }
    });
    mounted.push(wrapper);
    const component = child.vm as unknown as ListVM;
    await FLUSH();
  ...
  it('键盘：无历史时 Ctrl+Z / Ctrl+Shift+Z 不拦截（独立 RxDB 实例）', async () => {
    ...
    try {
      const { wrapper, child } = mountWithRxDB(EntityList, fresh, {
        props: { namespace: 'public', name: 'Todo' }
      });
      mounted.push(wrapper);
      const component = child.vm as unknown as ListVM;
      await FLUSH();
  ...
  it('onIconClicked：未知动作与未知记录 id 安全忽略', async () => {
    ...
    await component.onIconClicked({ name: 'other-action', record: { id: target.id } });
    await component.onIconClicked({ name: 'delete-action', record: { id: 'ghost-id' } });
    await FLUSH();
  ...
  it('M2M 确认：父实体关系 add + save；无关系属性时只 save；无父实体安全 no-op', async () => {
    ...
    await wrapper.setProps({ parentEntity: parent as never, parentRelationName: 'children' });
    await FLUSH();
    ...
    await wrapper.setProps({ parentEntity: parentNoRel as never });
    await FLUSH();
    component.onM2mSelectionConfirmed(component.tableRecords as never);
    await FLUSH();
    ...
    // 无父实体 / 无关系名：安全 no-op
    const { wrapper: loneWrapper, child: loneChild } = mountWithRxDB(EntityList, rxdb, {
      props: { namespace: 'public', name: 'Todo' }
    });
    mounted.push(loneWrapper);
    const loneComponent = loneChild.vm as unknown as ListVM;
    await FLUSH();
  ...
  it('M2M 确认：有父实体但无关系名时跳过关系处理', async () => {
    ...
    await wrapper.setProps({ parentEntity: parent as never });
    await FLUSH();
  ...
  it('openM2mSelectDialog：打开真实对话框、重入被挡、取消后可重开', async () => {
    ...
    // 打开真实对话框（DialogPortal 渲染 EntityDialog + 嵌套 EntityList）
    component.openM2mSelectDialog();
    await wrapper.vm.$nextTick();
    const dialogs = document.body.querySelectorAll('.rxdb-dialog-pane');
    expect(dialogs.length).toBe(1);
    expect(dialogs[0].textContent).toContain('添加Todo');

    // m2mDialogOpen 已持有 → 重入被挡
    component.openM2mSelectDialog();
    await wrapper.vm.$nextTick();
    expect(document.body.querySelectorAll('.rxdb-dialog-pane').length).toBe(1);

    // 取消关闭对话框后允许再次打开
    component.m2mCancelSelection();
    await wrapper.vm.$nextTick();
    await FLUSH();
    expect(document.body.querySelectorAll('.rxdb-dialog-pane').length).toBe(0);
  ...
  it('view-action 打开 edit 详情对话框并加载记录数据', async () => {
    ...
    await component.onIconClicked({ name: 'view-action', record });
    await vi.waitFor(() => {
      const dialog = document.body.querySelector('.rxdb-dialog-pane');
    ...
    await vi.waitFor(() => {
      const inputs = [...document.body.querySelectorAll<HTMLInputElement>('.rxdb-dialog-pane input')];
      expect(inputs.some(i => i.value === 'dialog-me')).toBe(true);
    });

    // 清理：点「取消」关闭对话框，避免污染后续用例的 overlay 断言
    const cancel = [...document.body.querySelectorAll<HTMLButtonElement>('.rxdb-dialog-pane button')].find(b =>
  ...
  it('editChain 含记录 id 时 view-action 只 emit 不打开编辑对话框（防环）', async () => {
    ...
    const { wrapper, child } = mountWithRxDB(EntityList, rxdb, {
      props: { namespace: 'public', name: 'Todo', editChain: [chained.id] }
    });
    mounted.push(wrapper);
    const component = child.vm as unknown as ListVM;
    await FLUSH();
    ...
    expect(((child.emitted('viewEntity') as unknown as unknown[][]) ?? []).map(e => e[0])).toEqual([record]);
    expect(document.body.querySelector('.rxdb-dialog-pane')).toBeNull();
  });
