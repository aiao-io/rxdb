...
/**
 * `SyncOptions` 联合里 QueryCache 的那一支。
 *
 * @remarks
 * 从联合里 `Extract` 而不是 import：那个成员接口在 `sync-options.interface.ts` 里没有导出，
 * 而它本就是配置面的内部形状。`Extract` 让它跟着联合一起变，不需要第二处维护。
 */
type QueryCacheSyncOptions = Extract<SyncOptions, { type: SyncType.QueryCache }>;
...
export class Repository<T extends EntityType, RT extends IRepository<T> = IRepository<T>> extends RepositoryBase<T> {
  /**
   * QueryCache 引擎会话；非 QueryCache 仓储为 `undefined`。
   *
   * @remarks
   * 「刚同步过」的记忆归它持有。存成字段而不是构造期的内联临时值，是因为远端失效上报要
   * **同步**清掉那份记忆（US-023 D2）。
   *
   * 引擎实现在 `@aiao/rxdb-plugin-querycache` 里（US-025 B1），这里只认
   * {@link QueryCacheSession} 这个接口 —— 核心不再进读路径那 1,541 行的依赖图。
   *
   * **首次订阅 `primary$` 时才建**，构造期不建：`VersionManager` 在 `new RxDB()` 里就会把
   * 系统树实体 `RxDBBranch` 的仓储构造出来，而插件要到 `connect()` 才安装。构造期取引擎
   * 等于让「库级 sync 声明 QueryCache」的配置在 `new RxDB()` 当场炸掉 —— 连装了插件的用法
   * 也一起炸，而且盖住了本该由 `init()` 报出的「树实体不支持 QueryCache」。
   */
  #queryCacheSession: QueryCacheSession<T> | undefined;

  /**
   * 本仓储是否走 QueryCache 策略。
   *
   * @remarks
   * 与 `#queryCacheSession !== undefined` 不是同一件事：会话是懒建的，订阅之前一直是
   * `undefined`。事件监听的注册/注销必须按**策略**判定，否则「构造后未订阅即 destroy()」
   * 会漏掉一次 `removeEventListener`，把死仓储钉在事件总线上。
   */
  readonly #isQueryCache: boolean;
   ...
   */
  #queryCachePrimary: QueryCachePrimary<T> | undefined;
  ...
  constructor(rxdb: RxDB, EntityType: T) {
    ...
    if (this.sync?.type === SyncType.QueryCache) {
      ...
      this.primary$ = this.#createQueryCachePrimary(metadata.namespace, metadata.name, this.sync) as Observable<RT>;
    } else {
   ...
   *
   * @remarks
   * `#queryCacheSession.clear()` 不是顺手做的清理，而是**释放资源**：记忆表的每一条都挂着一个
  ...
  destroy() {
    ...
    if (this.#isQueryCache) {
      ...
      // 从未订阅过 `primary$` 的仓储没有会话，也就没有计时器要撤
      this.#queryCacheSession?.clear();
    }
   ...
   * 作用域释放被卸掉，那时启动护栏早就过去了。两处共用一份消息构造。
   *
   * 只在**首次订阅**时调用，构造期不调：`new RxDB()` 会为系统树实体 `RxDBBranch` 建仓储，
  ...
  #resolveQueryCacheSession(namespace: string, entityName: string, sync: QueryCacheSyncOptions): QueryCacheSession<T> {
    ...
    const session = factory.createSession<T>({
      ...
      entityName,
      localCacheFirst: sync.local.localCacheFirst === true,
   ...
   * 断连重连后仍打向已断开的旧适配器。适配器能力校验由实现放在这次调用里，
   * 于是「缺 duck」在**首次真正用到它的调用**上抛出，而不是在配置期误伤。
   *
   * 正因为「每次发射重建」，同步记忆不能放在主仓储里 —— 订阅归零再订阅也会重建，
   * 那样记忆活不过一次 `find`。记忆因此归会话持有：适配器实例换了（断连重连，AC#22）
   * 即清空、没换则继续沿用，那次对表在 `createPrimary()` 实现里完成。
   *
   * 这层外壳留在核心，是因为它描述的是**适配器生命周期**而不是缓存语义：谁在什么时候
   * 重建、引用计数怎么算，是核心对两条适配器流的既有约定。
   */
  #createQueryCachePrimary(
    namespace: string,
    entityName: string,
    sync: QueryCacheSyncOptions
    ...
    return combineLatest([this.rxdb.localAdapter$, this.rxdb.remoteAdapter$]).pipe(
      map(([localAdapter, remoteAdapter]) => {
        const session = this.#resolveQueryCacheSession(namespace, entityName, sync);
        const primary = session.createPrimary(localAdapter, remoteAdapter);
