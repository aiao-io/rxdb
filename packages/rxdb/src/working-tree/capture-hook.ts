...
import { takeDeclaredWrite, type ResolvedTrustedWrite } from './trusted-write-scope.js';
...
/**
 * 全部系统实体名（`@Entity({ namespace: 'rxdb' })`）
 *
 * @remarks
 * 从 {@link SYSTEM_ENTITIES} 的元数据算出来，不另抄一份名字清单。抄一份的代价不是编译错误：
 * 新增一张系统表时，那份清单会安静地把它漏成业务实体，于是库自己的簿记写入开始被当成用户编辑
 * 落进工作树，而没有任何测试会因此变红。
 */
const SYSTEM_ENTITY_NAMES: ReadonlySet<string> = new Set(
  SYSTEM_ENTITIES.map(EntityType => getEntityMetadata(EntityType).name)
);
