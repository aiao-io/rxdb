...
import type { WorkingTreeCaptureHook } from '../capture-interceptor.js';
import { assertColdReplayInvariant, type ColdReplayRow, type ColdReplaySnapshot } from '../cold-replay.js';
import { gateRawWrite, judgeRawWrite } from '../raw-write-judgment.js';
import { TrustedWriteIntent } from '../trusted-write-intent.js';
import { declareTrustedWrite, type TrustedWriteDeclaration } from '../trusted-write-scope.js';
 ...
 *
 * @remarks
 * §1.4 有一半断言在**实体平面**（哪个实体归哪一类），而 `workingTreeRawWriteContext.domain`
 * 按类型只给表 / 列平面（{@link VersionedDomainView}）——实体归类只在钩子的
 * `targetClassOf()` 上。两个平面因此各有各的取法，不能互相替代。
 *
 * 钩子缺席时抛而不是跳过：`workingTree.enable()` 没生效的库上，本节全部断言都会以
 * 「归类为 versioned」的形态假绿。
 */
const hookOf = (adapter: LocalAdapter): WorkingTreeCaptureHook => {
...
const expectColdReplayIntact = async (database: RxDB, head: ColdReplaySnapshot = []): Promise<void> => {
  const adapter = await localAdapterOf(database);
  const exclusions = exclusionsOf(adapter.workingTreeRawWriteContext.domain);
  ...
  describe(`[${context.name}] 工作树捕获一致性`, () => {
    ...
    describe('§1.2 写入口语义矩阵', () => {
      ...
      it('未登记的入口默认拒绝：版本化目标上的放行集恰好是矩阵写明的那 5 行', async () => {
        ...
        const untrackedFields = [
          ...adapter.workingTreeRawWriteContext.domain.untrackedFieldsOf(getEntityMetadata(ConformanceNote).tableName)
        ];
    ...
    describe('§1.3 raw 通道 bypass 五步判定', () => {
      ...
      it('第 1 步 提交能力未启用：原样放行，零行为差异', async () => {
        ...
        // 能力位关掉的上下文只能现造：适配器在未启用时交出的那一份，`domain` 是个会抛的取值器
        // （`rxdb-adapter.ts` 的 CAPABILITY_DISABLED_RAW_WRITE_CONTEXT），展开它就抛了。
        expect(
          judgeRawWrite(sql, { capabilityEnabled: false, domain: adapter.workingTreeRawWriteContext.domain })
        ).toEqual({ kind: 'allow', step: 1, reason: 'capability_disabled' });
        await expectColdReplayIntact(database);
      });
      ...
      it('第 2 步 携带受信 intent：放行，且不必再看语句写了什么', async () => {
        ...
        expect(
          judgeRawWrite(sql, { ...adapter.workingTreeRawWriteContext, intent: TrustedWriteIntent.remote_sync })
      ...
      it('第 3 步 不是写语句：放行', async () => {
        ...
        expect(judgeRawWrite(sql, adapter.workingTreeRawWriteContext)).toEqual({
          kind: 'allow',
          step: 3,
          reason: 'not_a_write'
        });
      ...
      it('第 4 步 打在 tracked 列上：语句执行之前被拒，业务表零变化', async () => {
        const adapter = await localAdapterOf(database);
        const domain = adapter.workingTreeRawWriteContext.domain;
        const noteId = newEntityId();
        const fields = noteFields('第 4 步之前的值', null);
        const head = await materializeNote(noteId, fields);
        const sql = await trackedRawWrite();

        const judgment = judgeRawWrite(sql, adapter.workingTreeRawWriteContext);
      ...
      it('第 5 步 写域外目标：放行，理由是 out_of_domain', async () => {
        ...
        expect(
          judgeRawWrite(`UPDATE ${outOfDomainTable} SET title = '域外改的'`, adapter.workingTreeRawWriteContext)
        ).toEqual({ kind: 'allow', step: 5, reason: 'out_of_domain' });
        await expectColdReplayIntact(database);
      });
    });
