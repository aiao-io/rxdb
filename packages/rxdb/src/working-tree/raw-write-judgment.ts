 ...
 *
 * @remarks
 * **一份判定，六处调用。** 6 个适配器的 `rawQuery` 各自调这里；方言差异只落在词法归一化层。
 * 每个后端写一份的话，六份里只要有一份把归一化写松，整条防线就有洞——而那个洞不会在任何一个
 * 后端自己的测试里现形。
 *
 * **`rawQuery?()` 是可选方法**（`rxdb-adapter.ts:94`）。判定因此是**导出的纯函数**而不是基类钩子：
 ...
 * ```
 */
export function judgeRawWrite(sql: string, context: RawWriteContext): RawWriteJudgment {
