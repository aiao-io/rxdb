 ...
 *
 * 为什么不走占位符：`?`（SQLite 家族）与 `$1`（PGlite）方言不同，而这两条语句除**表引用**
 * 外在 6 个后端上是同一个字符串。一旦引入占位符，这里就得按后端分叉，判据也跟着分叉成 6 份。
 * 代价是值必须自己转义，所以本文件的每个函数都**拒绝**它转不动的输入，而不是尽力而为：
 * 拼 SQL 的地方一旦开始「尽力而为」，转义失败就会变成一条语法正确、语义错误的语句。
 *
 * 表引用是唯一的例外，而且**不由本文件拼**：PGlite 把 `namespace` 建成 schema
 * （`"rxdb"."rxdb_commit_capability"`），SQLite 家族把它拼成表名前缀
 * （`"rxdb$rxdb_commit_capability"`），两边都不等于 `metadata.tableName`。
 * 它由 `TransactionExecutor.tableRef()` 按后端解析后传进来 —— 命名规则属于适配器，
 * 在这里再抄一份就会抄错其中一边。{@link quoteSqlIdentifier} 只用于**列名**。
 *
