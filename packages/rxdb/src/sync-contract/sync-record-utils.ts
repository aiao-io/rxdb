...
export async function resolvePullIneligibility(
  ...
  return pullIneligibility(syncType, await findCurrentSyncRecord(rxdb, namespace, entity));
}
...
export async function resolvePushIneligibility(
  ...
  if (bySyncType) return bySyncType;
  return pushIneligibility(syncType, await findCurrentSyncRecord(rxdb, namespace, entity));
}
