 ...
 *
 * 「在依赖图上找出阻断者」那一半（`CascadeBlock` / `findBlockingDependency`）住在
 * `cascade-blocking.ts`：它要读依赖图，而依赖图是级联调度器的东西。US-025 阶段 C 之后
 * 调度器在 `@aiao/rxdb-plugin-history`，本文件留在核心供 QueryCache 出站队列判定资格，
 * 于是这条 `DependencyGraph` 依赖必须断在这里。
 */
