 ...
 * （`gateRawWrite`：证明拒绝发生在语句执行之前）。两个导出各有各的失败形态，合成一个就测不全。
 *
 * 为什么这些断言值得写：
 *
...
function rejectionFor(sql: string, ctx: RawWriteContext = context()) {
...
/** 判定并要求落在 allow 上。 */
function allowanceFor(sql: string, ctx: RawWriteContext = context()) {
  const judgment = judgeRawWrite(sql, ctx);
...
describe('挂载壳 gateRawWrite：拒绝发生在语句执行之前', () => {
  ...
  it('被拦时执行器一次都没被调用（业务表零变化）', async () => {
    ...
    await expect(gateRawWrite("UPDATE post SET title = 'x'", context(), probe.run)).rejects.toBeInstanceOf(
  ...
  it('抛出的错误带入口身份与错误码', async () => {
    const probe = executorProbe();
    const error = await gateRawWrite("UPDATE post SET title = 'x'", context(), probe.run).catch(
  ...
  it('放行时原样返回执行器的结果，只调用一次', async () => {
    ...
    const result = await gateRawWrite("UPDATE post SET remote_id = 'r1'", context(), probe.run);
  ...
  it('未启用能力时照常执行', async () => {
    ...
    await gateRawWrite("UPDATE post SET title = 'x'", context({ capabilityEnabled: false }), probe.run);
...
describe('挂载壳与纯判定是同一份规则', () => {
  ...
  it('语料上「壳抛没抛」与判定结论逐条相等', async () => {
    for (const sql of CORPUS) {
      ...
      const probe = { calls: 0 };
      const threw = await gateRawWrite(sql, context(), async () => {
        probe.calls += 1;
        return null;
      }).then(
        () => false,
