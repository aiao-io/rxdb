...
describe('query_merge_remove_cache', () => {
   ...
   * @param cachedById - 预置的实体缓存，用于复现乱序删除（RXD-018）
   */
  const createMockQueryTask = <RT>(
    ...
    createHarnessQueryTask(TestEntity, { ...queryOptions, runner, cachedById } as HarnessTaskOptions<
      TestEntityType,
      RT
    >);
  ...
  describe('RXD-023: count 删除路径关系门控缺失', () => {
    it('where 依赖关系字段时，删除当前实体不能靠扁平快照做 notExists 判断，必须走 SQL 刷新', () => {
      ...
      const task = createHarnessQueryTask<typeof User, number>(User, {
        type: 'count',
        options: {
          where: {
            combinator: 'and',
        ...
        schemaManager: {
          getEntityMetadata: (entity: string, namespace: string) =>
            entity === 'User' && namespace === 'test' ? userMetadata : undefined
        }
      } as unknown as HarnessTaskOptions<typeof User, number>);
