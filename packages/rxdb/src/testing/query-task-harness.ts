 ...
 *
 * @remarks
 * 不按 `task.type` 分派：那样测试台就得认识每一个查询类型的名字，而插件（树、图）
 * 会经 `RepositoryQueryExtensions` 往 `QueryOptions` 里并新类型——核心编译期根本看不见
 * `countDescendants` 这种名字，写进 `switch` 只会是类型错误，写进默认分支又会悄悄
 * 按数组算指纹而把计数结果算成空数组。结果形状是三类任务真正的分界，也不随插件变化：
 *
...
const harnessFingerprint = (result: unknown): Fingerprint[] => {
  ...
  return [JSON.stringify({ ...(result as object) })];
};
...
export const createHarnessQueryTask = <T extends EntityType, RT>(
  ...
  return queryManager.createTask<RT>({
    ...
    runner,
    getFingerprint: harnessFingerprint as (result: RT) => Fingerprint[]
  });
};
