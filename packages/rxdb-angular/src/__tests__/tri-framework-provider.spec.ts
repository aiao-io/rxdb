...
import { provideRxDB, useRxDB, useRxDBOptional, type RxDBSource } from '../index';
...
describe('三端 provider 契约', () => {
  ...
  describe('provider 入口接受同一个联合类型', () => {
    it('provideRxDB 收 RxDBSource', () => {
      const accept = (source: RxDBSource<RxDB>): EnvironmentProviders => provideRxDB(source);
