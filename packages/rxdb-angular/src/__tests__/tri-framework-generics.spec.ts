/**
 * 三端泛型契约（RAN-014）—— Angular 侧。
 *
 * @remarks
 * 断言全部在**编译期**成立：`@ts-expect-error` 由 `nx typecheck rxdb-angular`
...
describe('三端泛型契约（RAN-014）', () => {
  ...
  describe('无限滚动选项遵循实体声明的 findByCursorOptions', () => {
    it('通用 FindByCursorOptions 形状被编译期拒绝', () => {
      const options: InfiniteScrollOptions<typeof DeclaredCursorEntity> =
    ...
    it('实体声明的形状被接受', () => {
      const options: InfiniteScrollOptions<typeof DeclaredCursorEntity> = { bucket: 'inbox', limit: 10 };
      const factory: InfiniteScrollOptions<typeof DeclaredCursorEntity> = () => ({ bucket: 'inbox' });
